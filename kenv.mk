...
###############################
define cp-hct-config-to-kernel-config
startLine=`grep -n "hct_custom_config_begin" $(2) | head -1 | cut -d ":" -f 1`;\
if [ $$startLine -ne 0 ];then \
echo some hct_custom_config_begin find,$$startLine;\
endLine=`cat $(2) | wc -l`;\
sed -i "$$startLine"','$$endLine'd' $(2);\
fi;\
cat $(HCT_KERNEL_CONFIG_FILE)>>$(KERNEL_CONFIG_FILE)
endef
#########################
...
ifneq ($(strip $(TARGET_NO_KERNEL)),true)
  ...
  HCT_KERNEL_CONFIG_FILE:=$(KERNEL_DIR)/arch/$(KERNEL_TARGET_ARCH)/configs/hct_kernel_config
