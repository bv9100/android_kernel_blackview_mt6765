...
static int hct_finger_set_power_by_regulator(bool);
static struct regulator *fp_mt6370_ldo = NULL;
#endif
//add by micheal end 20181213
...
//#define RESTART_POWER
//#define LOWPOWER_CONFIG

#define VERSION                         "cdfinger version 4.1"
...
static uint8_t key_mode_flag = 0;  // for low power mode in key mode
static uint8_t key_mode_agc = 0;
...
static int cdfinger_parse_dts(struct cdfinger_data *cdfinger)
{
	...
	//cdfinger->spi->dev.of_node = of_find_compatible_node(NULL,NULL,"mediatek,fps1098");	changed by micheal 20181213
	...
	//cdfinger->cdfinger_irq = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_irq");	changed by micheal 20181213
	...
	//cdfinger->fps_reset_low = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_reset_low");	changed by micheal 20181213
	...
	//cdfinger->fps_reset_high = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_reset_high");	changed by micheal 20181213
	...
	if(cdfinger->config_spi_pin == 1)
	{
		...
		//cdfinger->cdfinger_spi_miso = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_spi_miso");	changed by micheal 20181213
		...
		//cdfinger->cdfinger_spi_mosi = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_spi_mosi");	changed by micheal 20181213
		...
		//cdfinger->cdfinger_spi_sck = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_spi_sck");	changed by micheal 20181213
		...
		//cdfinger->cdfinger_spi_cs = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_spi_cs");		changed by micheal 20181213
	...
	if(cdfinger->vdd_ldo_enable == 1)
	{
		...
		//cdfinger->fps_power_on = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_power_high");	changed by micheal 20181213
		...
		//cdfinger->fps_power_off = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_power_low");	changed by micheal 20181213
	...
	if(cdfinger->vio_ldo_enable == 1)
	{
		...
		//cdfinger->fps_vio_on = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_vio_high");	changed by micheal 20181213
		...
		//cdfinger->fps_vio_off = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_vio_low");	changed by micheal 20181213
...
#ifdef CONFIG_MT6370_PMU_LDO
	int ret =0;
	ret = hct_finger_set_power_by_regulator(true);
#endif
//add by micheal end 20181213
}
...
#ifdef CONFIG_MT6370_PMU_LDO
	int ret =0;
	ret = hct_finger_set_power_by_regulator(false);
#endif
//add by micheal end 20181213	
}
...
static void cdfinger_set_low_power_mode(struct cdfinger_data *cdfinger)
{
	...
	u8 read;
	int ret =0, count = 0;
	if (cdfinger->sensor_type != FPS998)
	{
		...
		for(count=0; count<5; count++)
		{
			...
			ret = spi_send_cmd_fifo(cdfinger, cdfinger->lp_reg.reg27,cdfinger->lp_reg.reg_rx,cdfinger->lp_reg.reg27_len);
			if (ret !=0){
			...
			ret = spi_send_cmd_fifo(cdfinger, cdfinger->lp_reg.reg21,cdfinger->lp_reg.reg_rx,cdfinger->lp_reg.reg21_len);
			if (ret !=0){
				CDFINGER_ERR("spi transfer failed! ret = %d\n", ret);
				continue;
			}
			...
			memset(cdfinger->lp_reg.reg_rx,0xff,16);
			ret = spi_send_cmd_fifo(cdfinger, cdfinger->lp_reg.reg22,cdfinger->lp_reg.reg_rx,cdfinger->lp_reg.reg22_len);
			if (ret !=0){
				CDFINGER_ERR("spi transfer failed! ret = %d\n", ret);
				continue;
			}
			...
			CDFINGER_REG("reg22: lp_reg[3] = 0x%x lp_reg[4] = 0x%x lp_reg[5] = 0x%x lp_reg[6] = 0x%x lp_reg[7]= 0x%x lp_reg[8]=0x%x\n", cdfinger->lp_reg.reg_rx[3], cdfinger->lp_reg.reg_rx[4], cdfinger->lp_reg.reg_rx[5], cdfinger->lp_reg.reg_rx[6], cdfinger->lp_reg.reg_rx[7],cdfinger->lp_reg.reg_rx[8]);

			if (cdfinger->lp_reg.reg_rx[6] != cdfinger->lp_reg.reg22[6])
			{
				CDFINGER_ERR("lp_register read value error!\n");
				continue;
			}
...
#else
	static u8 powerdown_cmd = 0x00, read;
...
static int cdfinger_chip_config(struct cdfinger_data * cdfinger)
{
	switch(cdfinger->sensor_type)
	{
		...
		case FPS958E:
			...
			break;
		case FPS980S:
			cdfinger->imageheight = 80;
			cdfinger->imagewidth = 80*2;
			cdfinger->imagebufsize = cdfinger->imageheight * cdfinger->imagewidth;
			cdfinger->imagegetsize = 50*1024;
			cdfinger->check_time = 5;
			break;
		default:
			goto exit;
	...
	switch(cdfinger->sensor_type)
	{
		...
		case FPS980S:
			...
			cdfinger->reg.reg22_len = 9;
			cdfinger->reg.reg27_len = 7;
			cdfinger->reg.reg28_len = 5;

			cdfinger->reg.reg21 = (u8*)kzalloc(cdfinger->reg.reg21_len*sizeof(u8),GFP_KERNEL);
			cdfinger->reg.reg22 = (u8*)kzalloc(cdfinger->reg.reg22_len*sizeof(u8),GFP_KERNEL);
			cdfinger->reg.reg27 = (u8*)kzalloc(cdfinger->reg.reg27_len*sizeof(u8),GFP_KERNEL);
			cdfinger->reg.reg28 = (u8*)kzalloc(cdfinger->reg.reg28_len*sizeof(u8),GFP_KERNEL);
			cdfinger->reg.image_agc = &cdfinger->reg.reg21[6];
			...
			cdfinger->reg.reg21[2] = 0x66;
			cdfinger->reg.reg21[3] = 0x36;
			cdfinger->reg.reg21[4] = 0x08;
			cdfinger->reg.reg21[5] = 0xfd;
			cdfinger->reg.reg21[6] = 0xe0;

			cdfinger->reg.reg22[0] = 0x22;
			cdfinger->reg.reg22[1] = 0x66;
			cdfinger->reg.reg22[2] = 0x66;
			cdfinger->reg.reg22[3] = 0x82;
			cdfinger->reg.reg22[4] = 0x00;
			cdfinger->reg.reg22[5] = 0x82;
			cdfinger->reg.reg22[6] = 0x00;
			cdfinger->reg.reg22[7] = 0x73;
			cdfinger->reg.reg22[8] = 0x00;

			cdfinger->reg.reg27[0] = 0x27;
			cdfinger->reg.reg27[1] = 0x66;
			cdfinger->reg.reg27[2] = 0x66;
			cdfinger->reg.reg27[3] = 0x24;
			cdfinger->reg.reg27[4] = 0x2c;
			cdfinger->reg.reg27[5] = 0x63;
			cdfinger->reg.reg27[6] = 0xf3;
			
			cdfinger->reg.reg28[0] = 0x28;
			cdfinger->reg.reg28[1] = 0x66;
			cdfinger->reg.reg28[2] = 0x66;
			cdfinger->reg.reg28[3] = 0x60;
			cdfinger->reg.reg28[4] = 0x39;
			break;
		default:
			goto exit;
	}
	if (cdfinger->sensor_type != FPS998)
	{
		cdfinger->lp_reg.reg21_len = cdfinger->reg.reg21_len;
		cdfinger->lp_reg.reg22_len = cdfinger->reg.reg22_len;
		cdfinger->lp_reg.reg27_len = cdfinger->reg.reg27_len;
		cdfinger->lp_reg.reg21 = (u8*)kzalloc(cdfinger->lp_reg.reg21_len*sizeof(u8),GFP_KERNEL);
		cdfinger->lp_reg.reg22 = (u8*)kzalloc(cdfinger->lp_reg.reg22_len*sizeof(u8),GFP_KERNEL);
		cdfinger->lp_reg.reg27 = (u8*)kzalloc(cdfinger->lp_reg.reg27_len*sizeof(u8),GFP_KERNEL);
		cdfinger->lp_reg.reg21[0] = 0x21;
		cdfinger->lp_reg.reg21[1] = 0x66;
		cdfinger->lp_reg.reg21[2] = 0x66;
		cdfinger->lp_reg.reg21[3] = 0x36;
		cdfinger->lp_reg.reg21[4] = 0x06;
		cdfinger->lp_reg.reg21[5] = 0xff;
		cdfinger->lp_reg.reg21[6] = 0xf0;

		cdfinger->lp_reg.reg22[0] = 0x22;
		cdfinger->lp_reg.reg22[1] = 0x66;
		cdfinger->lp_reg.reg22[2] = 0x66;
		cdfinger->lp_reg.reg22[3] = 0x00;
		cdfinger->lp_reg.reg22[4] = 0x00;
		cdfinger->lp_reg.reg22[5] = 0x00;
		cdfinger->lp_reg.reg22[6] = 0x00;
		cdfinger->lp_reg.reg22[7] = 0x00;
		cdfinger->lp_reg.reg22[8] = 0x00;

		cdfinger->lp_reg.reg27[0] = 0x27;
		cdfinger->lp_reg.reg27[1] = 0x66;
		cdfinger->lp_reg.reg27[2] = 0x66;
		cdfinger->lp_reg.reg27[3] = 0x3f;
		cdfinger->lp_reg.reg27[4] = 0x78;
		cdfinger->lp_reg.reg27[5] = 0xf8;
		cdfinger->lp_reg.reg27[6] = 0x12;
	}
	return 0;
...
static void cdfinger_register_init(struct cdfinger_data *cdfinger, int mode)
{
	switch(cdfinger->sensor_type)
	{
		...
		case FPS980S:
			...
			{
				cdfinger->reg.reg21[4] = 0x10;
				
				cdfinger->reg.reg27[3] = 0xa4;
				cdfinger->reg.reg27[4] = 0xcb;
				cdfinger->reg.reg27[5] = 0x62;
				cdfinger->reg.reg27[6] = 0x12;
				
				cdfinger->reg.reg28[4] = 0x01;
			}
			break;
	}
}
...
static int cdfinger_dev_init(struct cdfinger_data *cdfinger, u8 arg)
{
	...
	*cdfinger->reg.image_agc = arg;
	...
	for(count=0; count<5; count++)
	{
		...
		if(cdfinger->reg.reg28_len != 0)
		{
			ret = spi_send_cmd_fifo(cdfinger, cdfinger->reg.reg28,cdfinger->reg.reg_rx,cdfinger->reg.reg28_len);
			if (ret !=0){
				CDFINGER_ERR("spi transfer failed! ret = %d\n", ret);
				continue;
			}
...
static int cdfinger_check_interrupt(struct cdfinger_data *cdfinger)
{
	...
	int timeout = 40;
	int checktime = (1000/cdfinger->check_time)+50;
	CDFINGER_FUNCTION("enter %d\n", checktime);
		 ...
		 if((cdfinger->imagerxpix[pix_count] == head_sync[cdfinger->header-2])
			...
			if (linenum == cdfinger->imageheight-1){
				if(pix_count < cdfinger->imagebufsize)
					goto out;
				if (cdfinger->imagegetsize == 28*1024) {
					cdfinger->imagegetsize = (pix_count/1024+2)*1024;
					if (cdfinger->imagegetsize > 28*1024)
						cdfinger->imagegetsize = 28*1024;
				}
				CDFINGER_DBG("line:%d, pix count index:%d imagegetsize:%d\n",linenum, pix_count, cdfinger->imagegetsize);
				return 0;
			}
		}
	}
out:
	if(spi_send_cmd_fifo(cdfinger, &clcline_reg, &clcline_regstate, 1))
	{
		CDFINGER_ERR("cls failed!:%x\n",clcline_regstate);
	}
	cdfinger_reset(1);
	cdfinger->imagegetsize = 28*1024;
...
static int cdfinger_report_key(struct cdfinger_data *cdfinger, unsigned long arg)
{
...
static long cdfinger_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case CDFINGER_INIT:
			cdfinger->image_agc_ready = arg;
			CDFINGER_DBG("dev init fifo status %d\n",kfifo_is_empty(&cdfinger->image_fifo));
			if (kfifo_is_empty(&cdfinger->image_fifo) == 1) {
				CDFINGER_DBG("dev init fifo is empty\n");
		...
		case CDFINGER_GETIMAGE:
			...
			CDFINGER_DBG("get image fifo status %d\n",kfifo_is_empty(&cdfinger->image_fifo));
			if (kfifo_is_empty(&cdfinger->image_fifo) == 1) {
				CDFINGER_DBG("get image is empty\n");
		...
		case CDFINGER_INITERRUPT_MODE:
			key_mode_flag = 0;
			sign_sync = 0;
			kfifo_reset(&cdfinger->image_fifo);
		...
		case CDFINGER_INITERRUPT_FINGERUPMODE:
			...
			mod_timer(&cdfinger->int_timer, jiffies + HZ / cdfinger->check_time);
			break;
		...
		case CDFINGER_HW_RESET:
			cdfinger_reset(arg);
			break;
		case CDFINGER_INITERRUPT_KEYMODE_NEW:
			sign_sync = 0;
			key_mode_flag = 1;
			key_mode_agc = arg;
			kfifo_reset(&cdfinger->image_fifo);
...
static ssize_t cdfinger_read(struct file *file, char *buff, size_t count, loff_t * ppos)
{
	...
	wake_lock_timeout(&cdfinger->cdfinger_lock,3*HZ);
	mutex_lock(&cdfinger->buf_lock);
	if (kfifo_is_empty(&cdfinger->image_fifo) == 0) // not empty
	{
		CDFINGER_ERR("cdfinger read from fifo\n");
		ret = kfifo_out(&cdfinger->image_fifo, cdfinger->imagebuf, count);
	}
...
static int cdfinger_release(struct inode *inode, struct file *file)
{
	...
	CDFINGER_FUNCTION("enter cdfinger_release\n");
	if(cdfinger == NULL)
	{
		CDFINGER_ERR("%s: fingerprint please open device first!\n", __func__);
		return -EIO;
	...
	CDFINGER_FUNCTION("exit cdfinger_release\n");
	...
	do {
		...
		if (cdfinger->int_count >= cdfinger->interrupt_count) {
			...
			if (cdfinger->device_mode == CDFINGER_INTERRUPT_MODE) {
				...
				for (i = 0; i < image_ready_num; i ++)
				{
					ret = cdfinger_dev_init(cdfinger, cdfinger->image_agc_ready); 
					ret = cdfinger_read_image(cdfinger, 1);
					if(ret == 0) {
						CDFINGER_ERR("zard get image in thread %d\n", i);
						kfifo_in(&cdfinger->image_fifo, cdfinger->imagebuf, cdfinger->imagebufsize);
						CDFINGER_ERR("zard get image in thread status %d\n",kfifo_is_empty(&cdfinger->image_fifo));
						//up(&cdfinger->image_ready);
					} 
				}
				sign_sync = 1;
				wake_up_interruptible(&cdfinger_waitqueue);
...
static int cdfinger_check_id(struct cdfinger_data *cdfinger)
{
	...
	CDFINGER_PROBE("reg[5] = 0x%x reg[6] = 0x%x\n",id_rx[5], id_rx[6]);
	...
	id_cmd[0] = 0x74;
	spi_send_cmd_fifo(cdfinger,id_cmd,id_rx,sizeof(id_cmd)/sizeof(id_cmd[0]));
	CDFINGER_PROBE("reg[3] = 0x%x ",id_rx[3]);
	switch(id_rx[3])
	{
		case FPS998E:
		case FPS988E:
	...
	for(; speed>=CDFINGER_SPI_4M1; speed--)
	{
		...
		if(ret<(cdfinger->imagegetsize - 1500) && ret>cdfinger->imagebufsize)
		{
			...
			if(ret < 0 || ret > (cdfinger->imagegetsize - 1500))
				continue;
			cdfinger->imagegetsize = (ret/1024+2)*1024;
			if (cdfinger->imagegetsize > 28*1024)
				cdfinger->imagegetsize = 28*1024;
                                        ...
                                        unsigned long event, void* data)
{
    ...
    switch (blank) {
        case FB_BLANK_UNBLANK:
			...
			fb_status = 1;
			if (key_mode_flag != 0){ // key mode
				CDFINGER_DBG("sunlin===key mode resume(%x)\n",key_mode_flag);
				cdfinger_mode_init(g_cdfinger,key_mode_agc,CDFINGER_INTERRUPT_MODE);
			}
        ...
        case FB_BLANK_POWERDOWN:
			...
			fb_status = 0;
			if (key_mode_flag != 0){ // key mode
				CDFINGER_DBG("sunlin===FB_BLANK_POWERDOWN(%x)\n",key_mode_flag);
				cdfinger_set_low_power_mode(g_cdfinger);
			}
			mutex_unlock(&g_cdfinger->buf_lock);
            break;
...
//add by micheal start 20181213
#ifdef CONFIG_MT6370_PMU_LDO
static int hct_finger_set_power_by_regulator(bool on)
{
	int ret = 0;
	bool powerStatus = false;
	pr_err("hct %s: regulator_enable enable mt6370_ldo start!!\n", __func__);

	fp_mt6370_ldo = regulator_get(NULL, "irtx_ldo");

	if (IS_ERR(fp_mt6370_ldo)){
		ret = PTR_ERR(fp_mt6370_ldo);
		pr_err("hct %s: regulator_get mt6370_ldo fail=%d!!\n", __func__, ret);
		return -1;
	}
	else{
		...
		if (ret < 0) {
			pr_err("hct %s: regulator_set_voltage mt6370_ldo fail!!\n", __func__);
			return -1;
		}
		...
		if(on && !powerStatus) {
			...
			if (ret < 0) {
				pr_err("hct %s: regulator_enable enable mt6370_ldo fail!!\n", __func__);
				return -1;
			}
			mdelay(10);
			pr_err("hct %s: regulator_enable mt6370_ldo success!!\n", __func__);
		}else if(!on && powerStatus) {
			ret = regulator_disable(fp_mt6370_ldo);
			if (ret < 0) {
				pr_err("hct %s: regulator_enable enable mt6370_ldo fail!!\n", __func__);
				return -1;
			}
			pr_err("hct %s: regulator_disable mt6370_ldo success!!\n", __func__);
		}
...
static int cdfinger_probe(struct spi_device *spi)
{
	...
	hct_finger_set_spi_mode(1);//add by micheal 20181213
	hct_finger_set_eint(1);//add by micheal 20181213
	...
	status = kfifo_alloc(&cdfinger->image_fifo, cdfinger->imagebufsize *image_ready_num, GFP_KERNEL);// 2 * image buffer , means 2 finger buffers
	if (status) {
		CDFINGER_ERR("image fifo err(%d)\n",status);
                goto free_irq;
	}
...
static const struct of_device_id cdfinger_of_match[] = {
	...
	{ .compatible = "cdfinger,fps988e", },
	{ .compatible = "cdfinger,fps958e", },
	{},
};
...
static int __init cdfinger_spi_init(void)
{
	...
	err = spi_register_driver(&cdfinger_driver);
...
late_initcall(cdfinger_spi_init);	//add by micheal 20181213
