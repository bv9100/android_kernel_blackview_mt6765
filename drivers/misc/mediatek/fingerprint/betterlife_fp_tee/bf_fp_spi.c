...
int spi_dma_exchange(struct bf_device *bl229x, bl_read_write_reg_command_t read_write_cmd, int dma_size)
{
    ...
    spi_send_cmd(bl229x, read_write_cmd.data_tx, spi_rx_local_buf, dma_size);
...
int bf_read_chipid(void)
{
    ...
    if(g_chip_type == BF3290) {
        ...
        if(chip_id != BF3582P && chip_id != BF3582S && chip_id != 0x5883 && chip_id != 0x5683 && chip_id != 0x5a83) {
            //enable 0x10 bit5
            reg_value = bf_spi_read_reg(0x10);
            reg_value &= ~(1 << 5);
            bf_spi_write_reg(0x10, reg_value);
...
#if defined(PLATFORM_MTK) || defined(PLATFORM_MT6739)
...
#if 1//defined (COMPATIBLE)
    bf_chip_info();
    if(g_chip_type != 0x5183 && g_chip_type != 0x5283 && g_chip_type != 0x5383 && g_chip_type != 0x5483 && g_chip_type != 0x5783 && g_chip_type != 0x5883 && g_chip_type != 0x5683 && g_chip_type != 0x5a83) {
