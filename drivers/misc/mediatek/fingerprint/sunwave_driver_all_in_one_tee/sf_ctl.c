...
//---------------------------------------------------------------------------------
#define SF_DRV_VERSION "v2.2.34-2018-11-08"
...
extern int sf_ctl_device_power_by_regulator(bool on);
...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case SF_IOC_ENABLE_POWER: {
            // TODO:
            #if (SF_POWER_MODE_SEL == PWR_MODE_REGULATOR)
            sf_ctl_device_power_by_regulator(1);
            printk("hct_finger_boot vcc_enable\n");
            msleep(10);
            hct_finger_set_reset(1);
            printk("hct_finger_boot reset_enable\n");
            #endif
            break;
        }
        ...
        case SF_IOC_DISABLE_POWER: {
            ...
            printk("hct_finger_boot reset_disable\n");
            sf_ctl_device_power_by_regulator(0);
            printk("hct_finger_boot vcc_disable\n");
            #endif
            break;
        }
...
static int sf_read_sensor_id(void)
{
    ...
    do {
        ...
        //ret =  tee_spi_transfer(spi_master_id, &smt_conf, cfg_len, writebuf, readbuf, 6);
		...
		xprintk(SF_LOG_LEVEL,"readbuf[0] = 0x%x, readbuf[1] = 0x%x readbuf[2] = 0x%x readbuf[3] = 0x%x readbuf[4] = 0x%x readbuf[5] = 0x%x",readbuf[0], readbuf[1], readbuf[2], readbuf[3], readbuf[4], readbuf[5]);
        ...
        writebuf[3] = 0x02;
        writebuf[4] = 0x00;
        //ret =  tee_spi_transfer(spi_master_id, &smt_conf, cfg_len, writebuf, readbuf, 7);
        ...
        do {
            ...
            writebuf[3] = 0x00;
            writebuf[4] = 0x84;
            //ret =  tee_spi_transfer(spi_master_id, &smt_conf, cfg_len, writebuf, readbuf, 5);
            ...
            writebuf[6] = 0x00;
            writebuf[7] = 0x00;
            //ret =  tee_spi_transfer(spi_master_id, &smt_conf, cfg_len, writebuf, readbuf, 10);
...
static int __init sf_ctl_driver_init(void)
{
    ...
    hct = spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs));
...
late_initcall_sync(sf_ctl_driver_init);//clw add
#else
