...
static int sf_ctl_device_reset(void)
{
	...
	msleep(10);
	hct_finger_set_reset(1);
    xprintk(KERN_INFO, "hct_finger_set_reset is leave  !!!\n");
	return err;
#endif
#if SF_MTK_CPU
...
int sf_ctl_device_power_by_regulator(bool on)
{
...
//hct add 
#if 0//def CONFIG_MT6370_PMU_LDO
	pr_err("hctwang %s: regulator_enable enable mt6370_ldo start!!\n", __func__);
	fp_mt6370_ldo = regulator_get(NULL, "irtx_ldo");
//	ret = mtk_regulator_get(NULL, "irtx_ldo", &fp_mt6370_ldo);
//	if (ret < 0) {
	if (IS_ERR(fp_mt6370_ldo)){
		ret = PTR_ERR(fp_mt6370_ldo);
		pr_err("hctwang %s: regulator_get mt6370_ldo fail=%d!!\n", __func__, ret);
//		return -1;
	}
	else{
...
//		ret = mtk_regulator_set_voltage(&fp_mt6370_ldo, 2800000, 2800000);
		if (ret < 0) {
			pr_err("hctwang %s: regulator_set_voltage mt6370_ldo fail!!\n", __func__);
			return -1;
		}
		ret = regulator_enable(fp_mt6370_ldo);
//		ret = mtk_regulator_enable(&fp_mt6370_ldo, true);
		if (ret < 0) {
			pr_err("hctwang %s: regulator_enable enable mt6370_ldo fail!!\n", __func__);
			return -1;
		}
	
		pr_err("hctwang %s: regulator_enable enable mt6370_ldo success!!\n", __func__);
	}
#else
   // hct_finger_set_power(1);
#endif
...
//	sf_ctl_device_power(true);
	...
	ctl_dev->pdev->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,hct_finger");
	if (!ctl_dev->pdev->dev.of_node) {
	...
	xprintk(KERN_INFO, "irq number is %d.\n", ctl_dev->irq_num);
#if (SF_POWER_MODE_SEL == PWR_MODE_REGULATOR)
    ctl_dev->vdd_reg = regulator_get(&ctl_dev->pdev->dev, "irtx_ldo");
	printk("zcy add %s is regulator_get !!!\n",__func__);
...
#endif
	sf_ctl_device_power(true);
    xprintk(SF_LOG_LEVEL, "%s(..) ***** err = (%d)ok! exit.\n", __FUNCTION__,err);
	return err;
#endif
...
#endif
    xprintk(SF_LOG_LEVEL, "zcy add %s *******.\n", __FUNCTION__);
#if 0
#if (SF_POWER_MODE_SEL == PWR_MODE_REGULATOR)
...
int sf_platform_init(struct sf_ctl_device *ctl_dev)
{
	...
	if (hct_finger_probe_isok) {
		xprintk(KERN_ERR, "other fingerprint chip detect ok !\n");
		return -1;
	}
#endif
