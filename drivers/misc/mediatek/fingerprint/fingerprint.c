...
#ifdef CONFIG_FINGERPRINT_SUNWAVECORP_NEW_REE
		pinctrl_select_state(hct_finger_pinctrl, hct_finger_spi0_cs_as_gpio);
#else
...
printk("hubing 11111111111probe\n");
...
#ifndef CONFIG_OF
    int retval=0;
printk("hubing 11111111111init\n");
    retval = platform_device_register(&hct_finger_dev);
    if (retval != 0){
        return retval;
    }
#endif
printk("hubing 222222222init\n");
