...
static int set_speed = 1000000;//20180927
...
#define SPI_MODULE_CLOCK   (100*1000*1000)
	...
	set_speed = speed;//20180927
	xprintk(KERN_INFO, "spi speed is set to %d.\n", speed);//20180927
    if (time % 2) {
...
static int sf_read_sensor_id(void)
{
    ...
    do {
		...
		//printk("zcy add readbuf[2] = (%x) [3] = (%x) [4] = (%x) [5] = (%x)\n",readbuf[2],readbuf[3],readbuf[4],readbuf[5]);	
    ...
    while (trytimes--);
	xprintk(KERN_INFO, "zcy add %s is end !!\n",__func__);
    return -1;
}
...
static int sf_ctl_init_irq(void)
{
    ...
    err = request_irq(sf_ctl_dev.irq_num, sf_ctl_device_irq,
                      IRQF_TRIGGER_FALLING, "sf-irq", NULL);
...
#endif
    xprintk(KERN_INFO, "sunwave fingerprint device control driver registered and err = (%d)\n",err);
