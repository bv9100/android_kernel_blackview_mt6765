...
static const struct of_device_id madev_dt_ids[] = {
	{ .compatible = MA_DTS_NAME },
	{},
};
MODULE_DEVICE_TABLE(of, madev_dt_ids);
#endif
...
static struct mt_chip_conf smt_conf = {
    .setuptime=10,
    .holdtime=10,
#if defined(CONFIG_FINGERPRINT_MICROARRAY_A80T)||defined(CONFIG_FINGERPRINT_MICROARRAY_A80T_TEE)
...
struct mtk_regulator fp_rt5081_ldo;
#endif
#endif
...
void mas_enable_spi_clock(struct spi_device *spi){
	printk("hct add for mas_enable_spi_clock \n");
//	mt_spi_enable_master_clk(spi); 
}

void mas_disable_spi_clock(struct spi_device *spi){
	printk("hct add for mas_disable_spi_clock \n");
