...
//	if(ret<=0){
   if(ret){
    ...
    enable_irq(irq);
    enable_irq_wake(irq);//@20170707 modify by mtk for fix bug(deep sleep cannot resume)
    return ret;
}
...
int init_connect(void){
    ...
    pr_info("hctfinger-%s: init_connect-1 ret=%d, srxb[0]=0x%x, srxb[1]=0x%x, srxb[2]=0x%x, srxb[3]=0x%x", __func__, ret, srxb[0], srxb[1], srxb[2], srxb[3]);
    msleep(5);
    for(i=0; i<4; i++){
        stxb[0] = 0x00;//0x0c changed by micheal 20190107
        stxb[1] = 0xff;
        stxb[2] = 0xff;
        stxb[3] = 0xff;
        ret = mas_sync(stxb, srxb, 4);
        pr_info("hctfinger-%s: init_connect-2 ret=%d, srxb[0]=0x%x, srxb[1]=0x%x, srxb[2]=0x%x, srxb[3]=0x%x", __func__, ret, srxb[0], srxb[1], srxb[2], srxb[3]);
        if(ret!=0) MALOGW("do init_connect failed!");
        if(srxb[3] == 0x41 || srxb[3] == 0x45) return 1; //add srxb[3] == 0x45 by micheal for E064 20190107
    }
    return 0;   
}
