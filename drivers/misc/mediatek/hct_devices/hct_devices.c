...
#define HCT_VERSION "hct_39_P0MP1"
...
#if __HCT_CAM_OTP_PDAF_STATUS__
extern char hct_mcam_otp_status;
extern char hct_mcam_pdaf_status;
extern char hct_scam_otp_status;
#endif
static int hct_camera_info_print(struct seq_file *se_f)
{
...
#if __HCT_CAM_OTP_PDAF_STATUS__
						if(hct_mcam_otp_status)
							seq_printf(se_f, "\n\t\t\t\totp ");
						if(hct_mcam_pdaf_status)
							seq_printf(se_f, " pdaf ");
#endif						
                        seq_printf(se_f, "\n");
					}
...
static void init_camera_device(struct hct_camera_device_dinfo *pdevice, struct ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT* mCamera)
{
...
int hct_camera_device_add(struct ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT* mCamera, campatible_type isUsed)
{
    struct hct_type_info * camera_type_info;    
    struct hct_camera_device_dinfo * pcamera_device;
    int flag = -1;
...
//hct-drv add for show PA model by zhuguanglei 20180505 begin
#if defined(CONFIG_SND_SOC_AW87319) || defined(CONFIG_SND_SOC_AW87329)
    extern int pa_model;
#endif
//hct-drv add for show PA model by zhuguanglei 20180505 end
