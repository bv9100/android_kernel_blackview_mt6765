...
int typec_gpio_status;
int g_gpio_accdet_mic_switch = 0;
int typec_recheck_flag=1;
#define ACCDET_MIC_SWITCH_NAME  "gpio-accdet-mic-switch"
#endif
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	mutex_lock(&accdet_eint_irq_sync_mutex);
	if (eint_accdet_sync_flag) {
	    #if __HCT_TYPEC_ACCDET_MIC_SUPPORT__ // jay_zs
		...
		{  // in this case we need reset accdet and recheckit
				...
				//gpio_direction_output(g_gpio_accdet_mic_switch, 1);
				typec_gpio_status=gpio_get_value(g_gpio_accdet_mic_switch);
				pr_info("[Accdet] Headset has plugged in , typec_gpio_status=%d\n",typec_gpio_status);
				if(typec_gpio_status == 1){
						//typec_gpio_status = 0;
						gpio_set_value(g_gpio_accdet_mic_switch, 0);   //Set gpio num to switch mic or GND
				}else{
						//typec_gpio_status = 1;
						gpio_set_value(g_gpio_accdet_mic_switch, 1);
				}
				disable_accdet();
...
#if __HCT_TYPEC_ACCDET_MIC_SUPPORT__
int hct_accdet_mic_switch(struct device *dev)
{
	int ret = 0;
	//struct device_node *np = dev->of_node;
	struct device_node *node;
	pr_info("[%s] start..\n", __func__);
	node = of_find_compatible_node(NULL, NULL, "mediatek,pmic-accdet");
	if (node) {
        printk("%s: node not null.\n", __func__);
	}
	else {
		printk("%s: cannot get the node: 'mediatek,pmic-accdet'.\n", __func__);
		return -ENODEV;
	}
#if 0
	if (np) {
		ret = of_get_named_gpio(np, "gpio-accdet-mic-switch", 0);
		if (ret < 0)
			pr_err("%s: get accdet mic switch GPIO failed (%d)", __func__, ret);
		else
			g_gpio_accdet_mic_switch = ret;
	}
	else
		return -1;
#endif
	g_gpio_accdet_mic_switch = hct_parse_dts1(node, ACCDET_MIC_SWITCH_NAME, false);

	if (gpio_is_valid(g_gpio_accdet_mic_switch))
			pr_info("gpio number %d is valid\n", g_gpio_accdet_mic_switch);

	if (g_gpio_accdet_mic_switch != 0) {
		ret = gpio_request(g_gpio_accdet_mic_switch, "gpio-accdet-mic-switch");
		if (ret) {
			pr_err("%s : accdet mic switch gpio_request failed\n", __func__);
			return -ENODEV;
		}
		pr_info("%s : accdet mic switch GPIO = %d\n", __func__, g_gpio_accdet_mic_switch);
		ret = gpio_direction_output(g_gpio_accdet_mic_switch, 0);
		if (ret) {
			pr_err("%s : accdet mic switch gpio_direction_output failed\n",  __func__);
			return -ENODEV;
		}
		gpio_set_value(g_gpio_accdet_mic_switch, 0);
		pr_info("%s : accdet mic switch gpio_get_value = %d\n", __func__, gpio_get_value(g_gpio_accdet_mic_switch));
	}
	pr_info("[%s] end..\n", __func__);
	return 0;
}
