...
#endif
//hct drv add begin for 8613 flashlight
#ifdef CONFIG_MT6370_PMU_FLED
int mt6370_set_scenario(int scenario);
#endif
//hct drv add end for 8613 flashlight
/******************************************************************************
 * Definition
 *****************************************************************************/
		...
		const char *buf, size_t size)
{
	...
	struct flashlight_dev *fdev;
	int ret;
	int is_main_fl = 0; //chenyichong fix bug: main(ch0), sub(ch1), when turn-on main, we should not call turn-off sub.
	//or else in turn-off sub fuction, it will call ch0&ch1 set to off mode.
	

	//mt6370_set_scenario(2); //chenyichong moved to bottom.
	...
	//chenyichong add to fix bug: we set decouple in flashlight-device.c, we should also set here, or else fl not work.
#ifdef CONFIG_MT6370_PMU_FLED
	mt6370_set_scenario(2); //chenyichong moved after set_driver or else will be replaced.
#endif
	pr_debug("Bob.chen. 4444. \n" );
//hct-drv modify @20180403 begin

	ret = fl_set_level(fdev, fl_arg.level);
	fl_arg.channel = fdev->dev_id.channel;
	fl_dev_arg.channel = fdev->dev_id.channel;
	fl_dev_arg.arg = 0;
	pr_debug("Bob.chen. 5555. \n" );
	
	fdev->ops->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, (unsigned long)&fl_dev_arg);
#if defined(CONFIG_MTK_FLASHLIGHT_AW3641)
        fdev->ops->flashlight_ioctl(FLASH_IOC_SET_DUTY, (unsigned long)&fl_dev_arg);//hct-drv add for aw3641 8613
#endif

	pr_debug("Bob.chen. 9999. \n" );

	ret = fl_enable(fdev, 1);
	
	pr_debug("Bob.chen. aaaa. \n" );

	if(fl_arg.level <= 0)
	{
		fdev->ops->flashlight_set_driver(0);
//fix 8613 can not close flashlight begin
        ret = fl_enable(fdev, 0);
//fix 8613 can not close flashlight end
		fdev->ops->flashlight_release();
#ifdef CONFIG_MT6370_PMU_FLED
	mt6370_set_scenario(0); //jsx add
#endif
	}
unlock:
	ret = size;
