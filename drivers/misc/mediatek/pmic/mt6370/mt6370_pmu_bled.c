...
//hct_drv add begin
#if defined(__HCT_BLED_BRIGHTNESS_CURRENT__)
#if __HCT_BLED_BRIGHTNESS_CURRENT__
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
#define AUX_IN0_LCD_ID  2
#define ADC_TEMP_VALUE   0x400
static int hct_bled_chan_en=3;
void hct_set_bled_brightness_chan_en(void);
int hct_get_bled_brightness_chan_en(void);
void hct_set_bled_brightness_chan_en(void)
{
	int adcdata[4] = {0};
    int rawdata = 0;
...
#if defined(__HCT_BLED_BRIGHTNESS_CURRENT__)
#if __HCT_BLED_BRIGHTNESS_CURRENT__
		hct_set_bled_brightness_chan_en(); 
		pdata->chan_en = hct_get_bled_brightness_chan_en();
#else
