...
#ifdef CONFIG_MTK_GSENSORHUB_DIRECTION//1	//DYM gsensor direction by gpio state
extern int gsensor_direction_for_gpio(void);
static int dym_direction = 0;
#define HUB_ACC_AXIS_X 0
#define HUB_ACC_AXIS_Y 1
#define HUB_ACC_AXIS_Z 2
#endif
...
#ifdef CONFIG_MTK_GSENSORHUB_DIRECTION//1//DYM gsensor direction by gpio state
	hwmsen_get_convert(dym_direction, &cvttemp);

	raw_data[HUB_ACC_AXIS_X] = data.accelerometer_t.x;
	raw_data[HUB_ACC_AXIS_Y] = data.accelerometer_t.y;
	raw_data[HUB_ACC_AXIS_Z] = data.accelerometer_t.z;

	remap_data[cvttemp.map[HUB_ACC_AXIS_X]] = cvttemp.sign[HUB_ACC_AXIS_X] * raw_data[HUB_ACC_AXIS_X];
	remap_data[cvttemp.map[HUB_ACC_AXIS_Y]] = cvttemp.sign[HUB_ACC_AXIS_Y] * raw_data[HUB_ACC_AXIS_Y];
	remap_data[cvttemp.map[HUB_ACC_AXIS_Z]] = cvttemp.sign[HUB_ACC_AXIS_Z] * raw_data[HUB_ACC_AXIS_Z];

	acc[ACCELHUB_AXIS_X] = remap_data[HUB_ACC_AXIS_X];
	acc[ACCELHUB_AXIS_Y] = remap_data[HUB_ACC_AXIS_Y];
	acc[ACCELHUB_AXIS_Z] = remap_data[HUB_ACC_AXIS_Z];
#else
...
static int gsensor_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	raw_data[HUB_ACC_AXIS_X] = event->accelerometer_t.x;
    	raw_data[HUB_ACC_AXIS_Y] = event->accelerometer_t.y;
    	raw_data[HUB_ACC_AXIS_Z] = event->accelerometer_t.z;

	remap_data[cvttemp.map[HUB_ACC_AXIS_X]] = cvttemp.sign[HUB_ACC_AXIS_X] * raw_data[HUB_ACC_AXIS_X];
	remap_data[cvttemp.map[HUB_ACC_AXIS_Y]] = cvttemp.sign[HUB_ACC_AXIS_Y] * raw_data[HUB_ACC_AXIS_Y];
	remap_data[cvttemp.map[HUB_ACC_AXIS_Z]] = cvttemp.sign[HUB_ACC_AXIS_Z] * raw_data[HUB_ACC_AXIS_Z];

	data.x = remap_data[HUB_ACC_AXIS_X];
	data.y = remap_data[HUB_ACC_AXIS_Y];
	data.z = remap_data[HUB_ACC_AXIS_Z];
#else
