...
static int mmc3680x_suspend(struct device *dev);
static int mmc3680x_resume(struct device *dev);
...
static const struct dev_pm_ops mmc3680x_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(mmc3680x_suspend, mmc3680x_resume)
};
#endif
...
static int mag_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	printk("----dong:mag_i2c_read_block 111, msgs[1].buf[0] = [0x%x], msgs[1].buf[1] = [0x%x]----\n",msgs[1].buf[0],msgs[1].buf[1]);
	if (!client) {
	...
	mutex_unlock(&mmc3680x_i2c_mutex);
	printk("----dong:mag_i2c_read_block 222, msgs[1].buf[0] = [0x%x], msgs[1].buf[1] = [0x%x]----\n",msgs[1].buf[0],msgs[1].buf[1]);
	return err;
...
static int I2C_RxData(char *rxData, int length)
{
	...
	printk("----dong:I2C_RxData 111, addr = [0x%x]----\n",addr);
	...
	printk("----dong:I2C_RxData 222, addr = [0x%x]----\n",addr);
...
static int mmc3680x_read_otp(void)
{
	int stemp = 0;
	char reg[2] = {0};
	unsigned char otpdata[2] = {0};

	reg[0] = MMC3680X_MAG_REG_ADDR_OTP_GATE0;
	reg[1] = MMC3680X_MAG_OTP_OPEN0;
	I2C_TxData(reg, 2);

	reg[0] = MMC3680X_MAG_REG_ADDR_OTP_GATE1;
	reg[1] = MMC3680X_MAG_OTP_OPEN1;
	I2C_TxData(reg, 2);
	
	reg[0] = MMC3680X_MAG_REG_ADDR_OTP_GATE2;
	reg[1] = MMC3680X_MAG_OTP_OPEN2;
	I2C_TxData(reg, 2);

	reg[0] = MMC3680X_MAG_REG_ADDR_CTRL2;
	reg[1] = MMC3680X_MAG_OTP_OPEN2;
	I2C_TxData(reg, 2);

	otpdata[0] = MMC3680X_MAG_REG_ADDR_OTP;
	I2C_RxData(otpdata, 2);
	
	reg[0] = MMC3680X_MAG_REG_ADDR_CTRL2;
	reg[1] = MMC3680X_MAG_OTP_CLOSE;
	I2C_TxData(reg, 2);
...
static int mmc3680x_processor_otp(int* in, int* out)
{
...
static int ECS_ReadXYZData(int *vec, int size)
{
	...
	mmc3680x_processor_otp(magraw,vec);
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		MEMSIC_ERR("invalid content: '%s', length = %zu\n", buf, count);
	}
...
static int check_device(void)
{
	...
	product_id[0] = MMC3680X_REG_PRODUCTID;	//0x2F
	...
	product_id[1] = MMC3680X_REG_SUB_PRODUCTID;	//0X39
	if(I2C_RxData(&product_id[1], 1) < 0)
	{
		MEMSIC_ERR("[mmc3680x] read id fail\n");
		//read again
		I2C_RxData(&product_id[1], 1);
	}

	MEMSIC_INFO("[mmc3680x] product_id[0] = 0x%x  product_id[1] = 0x%x\n", product_id[0], product_id[1]);
	if((product_id[1] != 0x10)&&(product_id[0] == MMC3680x_DEVICE_ID))
	{
		MEMSIC_ERR("Got memsic mmc3680x id successful\n");
	    return 0;
	}
    else
	{
...
static int mmc3680x_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
static int mmc3680x_set_delay(u64 ns)
{
    int value=0;
...
static int mmc3680x_open_report_data(int open)
{
	return 0;
}

static int mmc3680x_get_data(int* x ,int* y,int* z, int* status)
{
	int data[3] = {0};
	...
	*z = data[2];
	*status = 3;
	MEMSIC_INFO("memsic 3680 data x/y/z/status=%d %d %d %d\n",*x,*y,*z,*status);
	return 0;
}
...
static int mmc3680x_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;

	err = mmc3680x_enable(enabledisable == true ? 1 : 0);
	if (err) {
		MEMSIC_ERR("%s enable sensor failed!\n", __func__);
		return -1;
	}
	err = mmc3680x_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int mmc3680x_factory_get_data(int32_t data[3], int *status)
{
	/* get raw data */
	return  mmc3680x_get_data(&data[0], &data[1], &data[2], status);
}
static int mmc3680x_factory_get_raw_data(int32_t data[3])
{
	MEMSIC_INFO("do not support mmc3680x_factory_get_raw_data!\n");
	return 0;
}
static int mmc3680x_factory_enable_calibration(void)
{
	return 0;
}
static int mmc3680x_factory_clear_cali(void)
{
	return 0;
}
static int mmc3680x_factory_set_cali(int32_t data[3])
{
	return 0;
}
static int mmc3680x_factory_get_cali(int32_t data[3])
{
	return 0;
}
static int mmc3680x_factory_do_self_test(void)
{
	return 0;
}

static struct mag_factory_fops mmc3680x_factory_fops = {
	.enable_sensor = mmc3680x_factory_enable_sensor,
	.get_data = mmc3680x_factory_get_data,
	.get_raw_data = mmc3680x_factory_get_raw_data,
	.enable_calibration = mmc3680x_factory_enable_calibration,
	.clear_cali = mmc3680x_factory_clear_cali,
	.set_cali = mmc3680x_factory_set_cali,
	.get_cali = mmc3680x_factory_get_cali,
	.do_self_test = mmc3680x_factory_do_self_test,
};

static struct mag_factory_public mmc3680x_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &mmc3680x_factory_fops,
};
...
static int mmc3680x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mag_data.get_data = mmc3680x_get_data;
