...
static int gyro_dym_direction = 0;
#endif
...
#ifdef CONFIG_MTK_GSENSORHUB_DIRECTION//1//DYM gsensor direction by gpio state
#define HUB_GRY_AXIS_X 0
#define HUB_GRY_AXIS_Y 1
#define HUB_GRY_AXIS_Z 2
#endif
...
#ifdef CONFIG_MTK_GSENSORHUB_DIRECTION//1//DYM gsensor direction by gpio state
	hwmsen_get_convert(gyro_dym_direction, &cvttemp);

	raw_data[HUB_GRY_AXIS_X] = event->gyroscope_t.x;
    	raw_data[HUB_GRY_AXIS_Y] = event->gyroscope_t.y;
    	raw_data[HUB_GRY_AXIS_Z] = event->gyroscope_t.z;

	remap_data[cvttemp.map[HUB_GRY_AXIS_X]] = cvttemp.sign[HUB_GRY_AXIS_X] * raw_data[HUB_GRY_AXIS_X];
	remap_data[cvttemp.map[HUB_GRY_AXIS_Y]] = cvttemp.sign[HUB_GRY_AXIS_Y] * raw_data[HUB_GRY_AXIS_Y];
	remap_data[cvttemp.map[HUB_GRY_AXIS_Z]] = cvttemp.sign[HUB_GRY_AXIS_Z] * raw_data[HUB_GRY_AXIS_Z];

	data.x = remap_data[HUB_GRY_AXIS_X];
	data.y = remap_data[HUB_GRY_AXIS_Y];
	data.z = remap_data[HUB_GRY_AXIS_Z];
#else
...
static int gyrohub_probe(struct platform_device *pdev)
{
	...
	if(gsensor_direction_for_gpio()){
		gyro_dym_direction = 1;
	}
	else{
		gyro_dym_direction = 6;
	}
#endif
