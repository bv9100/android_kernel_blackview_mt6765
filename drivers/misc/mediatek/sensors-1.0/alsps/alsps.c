...
static int alsps_real_driver_init(void)
{
	int i = 0;
	int err = 0;
	int count = 0; //add by micheal for als init 20190322
...
//hct-drv bob.chen modify for Discrete ALSPS begin.
#if defined(CONFIG_DISCRETEALSPS) 
		err =  ret; //0; //-1; //bob.chen modify. need ret 0, otherwise it will reboot.
#else
		err = -1;
#endif
//hct-drv bob.chen modify for Discrete ALSPS end.
...
static DISCRETEALSPS_DIRVER *DiscreteALSPS_func_list[DISCRETEALSPS_MAX] = {0};
static int DiscreteALSPS_init_done_list[DISCRETEALSPS_MAX] = {0};

int DiscreteALSPS_add(DISCRETEALSPS_INDEX index, DISCRETEALSPS_DIRVER *func)
{
	int err = 0;
    
	if (index >= DISCRETEALSPS_MAX) {
		pr_err("ALSPS DiscreteALSPS_add add err\n");
		err = -1;
	}

        DiscreteALSPS_func_list[index] = func;

	pr_err("%s. \n", __func__);

	return err;
}
EXPORT_SYMBOL_GPL(DiscreteALSPS_add);

DISCRETEALSPS_DIRVER* DiscreteALSPS_func(DISCRETEALSPS_INDEX index)
{
        if (index >= DISCRETEALSPS_MAX) {
            pr_err("ALSPS DiscreteALSPS_func. error. \n");
            index = DISCRETEALSPS_MAX - 1;
        }
        pr_err("ALSPS DiscreteALSPS_func. \n");

        return DiscreteALSPS_func_list[index];
}
EXPORT_SYMBOL_GPL(DiscreteALSPS_func);


int DiscreteALSPS_init_done(DISCRETEALSPS_INDEX index, int is_done)
{
	int err = 0;
	...
	if (index >= DISCRETEALSPS_MAX) {
        ...
        DiscreteALSPS_init_done_list[index] = is_done;

	pr_err("%s. \n", __func__);

	return err;
}
EXPORT_SYMBOL_GPL(DiscreteALSPS_init_done);

int DiscreteALSPS_is_allowed_probe(void)
{    
	pr_err("%s.  ALS: %d.  PS: %d. \n", __func__, DiscreteALSPS_init_done_list[DISCRETEALSPS_ALS] , DiscreteALSPS_init_done_list[DISCRETEALSPS_PS] );
    
        if(DiscreteALSPS_init_done_list[DISCRETEALSPS_ALS] && DiscreteALSPS_init_done_list[DISCRETEALSPS_PS]){
            return 1;
        }
        else{
            return 0;
...
EXPORT_SYMBOL_GPL(DiscreteALSPS_is_allowed_probe);
