...
#define STK_HT_N_CT				80
#endif /* #ifdef STK_TUNE0 */

static int fae_count = 0;
...
extern struct i2c_client *stk3x3x_als_i2c_client;		//add by micheal for als ps compatible 20190322
/*----------------------------------------------------------------------------*/
static const struct i2c_device_id stk3x3x_i2c_id[] = { {stk3x3x_DEV_NAME, 0}, {} };
//static struct regulator *stk3x3x_regulator;
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
//static int stk3x3x_i2c_suspend(struct i2c_client *client, pm_message_t msg);
//static int stk3x3x_i2c_resume(struct i2c_client *client);
//static struct stk3x3x_priv *g_stk3x3x_ptr = NULL;
...
// static struct sensor_init_info stk3x3x_init_info = {
...
//static int32_t stk3x3x_get_ir_value(struct stk3x3x_priv *obj, int32_t als_it_reduce);
#ifdef STK_ALS_MID_FIR
...
static int stk3x3x_init_als_client(struct i2c_client *client);	//defined by micheal for als init 20190322
...
int stk3x3x_get_addr( struct stk3x3x_i2c_addr *addr)
{
	...
	addr->pd_selc = STK_PD_SELECT_REG;
	addr->a_gain = STK_AGAIN_REG;
...
static int stk3x3x_otp_read_byte_data(struct i2c_client *client, unsigned char command)
{
	...
	u8 data;
	data = 0x2;
	ret = stk3x3x_master_send(client, 0x0, &data, 1);
	...
	data = command;
	ret = stk3x3x_master_send(client, 0x90, &data, 1);
	...
	data = 0x82;
	ret = stk3x3x_master_send(client, 0x92, &data, 1);
	...
	ret = stk3x3x_master_recv(client, 0x91, &data, 1);
...
/*----------------------------------------------------------------------------*/
#ifdef STK_CHK_REG
static int als_check_reg_count = 0;
#endif
int stk3x3x_read_als(struct i2c_client *client, u16 *data)
{
	...
	if (als_check_reg_count == 15)
	{
		err = stk3x3x_validate_n_handle(client);	//changed by micheal for als ps compatible 20190322
		if(err < 0)
		{
			APS_ERR("stk3x3x_validate_n_handle fail: %d\n", err);
		}

		if (obj->als_enable == false)
		{
			err = stk3x3x_write_state(client, 0x02);
			if (err < 0)
				return err;
			else
				atomic_set(&obj->state_val, 0x02);
			obj->als_enable = true;
		}
	}
	else if(als_check_reg_count > 15)
	{
		als_check_reg_count = 0;
	}
#endif

	ret = stk3x3x_master_recv(client, obj->addr.data1_als, buf, 0x08);

	if (ret < 0)
	{
		printk("stk3x3x_read_als error: %d\n", ret);
		return -EFAULT;
	}

	ret = stk3x3x_master_recv(client, STK_DATA1_C_REG, buf_c, 0x02);
	if (ret < 0)
	{
		printk("stk3x3x_read_als error: %d\n", ret);
		return -EFAULT;
...
#ifdef STK_ALS_MID_FIR
	printk("stk3x3x_read_als STK_ALS_MID_FIR\n");
...
/*----------------------------------------------------------------------------*/
#if 0
static void stk3x3x_power(struct alsps_hw *hw, unsigned int on)
{
#if 0
	int ret;
	...
	if (on == 1)
	{
		ret = regulator_enable(stk3x3x_regulator);	/*enable regulator*/
	...
	{
		ret = regulator_disable(stk3x3x_regulator);       /*enable regulator*/
...
static int stk3x3x_enable_ps_set_thd(struct stk3x3x_priv *obj)
{
	...
	if (obj->crosstalk > 0)
	{
		atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot + 300);
		atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot + 200);
	}
...
static int stk3x3x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(enable)
	{
		err = stk3x3x_validate_n_handle(obj->client);
		if(err < 0)
		{
			APS_ERR("stk3x3x_validate_n_handle fail: %d\n", err);
		}
...
static int stk3x3x_ps_calibration(struct i2c_client *client)
{
	...
	if (org_state_reg & STK_STATE_EN_ALS_MASK)
	{
		...
		stk3x3x_enable_als(stk3x3x_als_i2c_client, 0);	//changed by micheal for als ps compatible 20190322
		re_en_als = true;
	}
...
err_out:
	...
	if (re_en_als)
	{
		...
		stk3x3x_enable_als(stk3x3x_als_i2c_client, 1);	//changed by micheal for als ps compatible 20190322
	}
...
#ifdef STK_CHK_REG
static int stk3x3x_chk_reg_valid(struct stk3x3x_priv *obj)
{
	int ret = 0;
	u8 buf[9];
	if(NULL == obj)
	...
	ret = stk3x3x_master_recv(stk3x3x_obj->client, 1, &buf[0], 7);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
static int stk3x3x_validate_n_handle(struct i2c_client *client)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);
	int err;
	err = stk3x3x_chk_reg_valid(obj);
	if(err < 0)
	{
		APS_ERR("stk3x3x_chk_reg_valid fail: %d\n", err);
		return err;
	}
	if(err == 0xFF)
	{
		APS_ERR("%s: Re-init chip\n", __func__);
		stk3x3x_init_client(client);	//changed by micheal for als ps compatible 20190322
...
static int stk3x3x_set_als_int_thd(struct i2c_client *client, u16 als_data_reg)
{
	...
	APS_LOG("stk3x3x_set_als_int_thd:als_thd_h:%d,als_thd_l:%d\n", als_thd_h, als_thd_l);
...
static int stk_ps_tune_zero_func_fae(struct stk3x3x_priv *obj)
{
	...
	if ((obj->psi_set != 0))
	{
		...
		{
			if (word_data < atomic_read(&obj->ps_low_thd_val))
			{
				stk_ps_report(obj, 1);
			}
			else if ((word_data > (obj->psi + STK_H_PS)) && !(obj->ps_thd_update))
			{
				...
				APS_LOG("%s:Ling3 ps update ps = %d, psi = %d\n", __FUNCTION__, word_data, obj->psi);
...
static void stk3x3x_eint_work(struct work_struct *work)
{
	...
	if (((1 << STK_BIT_ALS) & obj->pending_intr) && (obj->hw.polling_mode_als == 0))
	{
		...
		if ((err = stk3x3x_read_als(stk3x3x_als_i2c_client, &obj->als)))	//changed by micheal for als ps compatible 20190322
		{
	...
	if (((1 << STK_BIT_PS) &  obj->pending_intr) && (obj->hw.polling_mode_ps == 0))
	{
		...
		if (near_far_state == 0)
		{	
			if (!(obj->ps_thd_update) && obj->ps > (obj->psi + STK_H_PS))
			{
				atomic_set(&obj->ps_high_thd_val, obj->psi + STK_H_HT);
				atomic_set(&obj->ps_low_thd_val, obj->psi + STK_H_LT);
...
//	struct pinctrl_state *pins_default;
	...
	if (stk3x3x_obj->irq_node)
	{
		...
		enable_irq_wake(stk3x3x_obj->irq); //@20170707 modify by mtk for fix bug(deep sleep cannot resume)
	}
	else
	{
...
static int stk3x3x_init_client(struct i2c_client *client)
{
	...
	/*
		if((err = stk3x3x_check_intr(client, &int_status)))
		{
			APS_ERR("check intr error: %d\n", err);
			return err;
		}

		if((err = stk3x3x_clear_intr(client, int_status, STK_FLG_PSINT_MASK | STK_FLG_ALSINT_MASK)))
		{
	...
	*/
	...
	i2c_config_reg = 0xf; 
	ret = stk3x3x_master_send(client, obj->addr.pd_selc, &i2c_config_reg, 1);
	if (ret < 0)
	{
		return -EFAULT;
	}
	i2c_config_reg = 0x15;
	ret = stk3x3x_master_send(client, obj->addr.a_gain, &i2c_config_reg, 1);
	if (ret < 0)
	{
		return -EFAULT;
	}
	i2c_config_reg = 0x01;
	ret = stk3x3x_master_send(client, 0xFA, &i2c_config_reg, 1);
	if (ret < 0)
	{
		return -EFAULT;
	}
	i2c_config_reg = 0x0;
	ret = stk3x3x_master_send(client, 0xF4, &i2c_config_reg, 1);
	if (ret < 0)
	{
		return -EFAULT;
	}
	...
	u8 data;
	data = 0x60;
	err = stk3x3x_master_send(client, 0x87, &data, 1);
	if (err < 0)
	{
...
//defined by micheal for als init start 20190322
static int stk3x3x_init_als_client(struct i2c_client *client)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);
	int err;
	int ret;
...
//add by micheal for als init end 20190322
...
static ssize_t stk3x3x_show_als(struct device_driver *ddri, char *buf)
{
	...
	{
		if ((res = stk3x3x_read_als(stk3x3x_als_i2c_client, &stk3x3x_obj->als)))	//changed by micheal for als ps compatible 20190322
...
static ssize_t stk3x3x_show_reg(struct device_driver *ddri, char *buf)
{
	...
	//stk3x3x_clear_intr(stk3x3x_obj->client, int_status, 0x0);
	stk3x3x_read_ps(stk3x3x_obj->client, &stk3x3x_obj->ps);
	stk3x3x_read_als(stk3x3x_als_i2c_client, &stk3x3x_obj->als);	//changed by micheal for als ps compatible 20190322
	/*write*/
	stk3x3x_write_als(stk3x3x_als_i2c_client, atomic_read(&stk3x3x_obj->alsctrl_val));	//changed by micheal for als ps compatible 20190322
...
static ssize_t stk3x3x_show_status(struct device_driver *ddri, char *buf)
{
	...
	/*
	ret = stk3x3x_master_recv(stk3x3x_obj->client, 21, &rbuf[21], 4);
	if(ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
{
	// struct stk3x3x_priv *data = i2c_get_clientdata(stk3x3x_obj->client);
#ifdef CALI_EVERY_TIME
...
static int stk3x3x_get_ps_value(struct stk3x3x_priv *obj, u16 ps)
{
	...
	/*
	if((err = stk3x3x_clear_intr(obj->client, flag, STK_FLG_OUI_MASK)))
	{
		APS_ERR("fail: %d\n", err);
		return err;
	}
...
{
    //hct-drv bob.chen modify for Discrete ALSPS begin.
    #if defined(CONFIG_DISCRETEALSPS)
        struct i2c_client *client = stk3x3x_i2c_client;
    #else
        struct i2c_client *client = (struct i2c_client*)file->private_data;
    #endif
    //hct-drv bob.chen modify for Discrete ALSPS end.
	...
	case ALSPS_GET_PS_DATA:
		...
		dat = stk3x3x_get_ps_value(obj, obj->ps);
	...
	case ALSPS_SET_ALS_MODE:
		...
		if (enable)
		{
			if ((err = stk3x3x_enable_als(stk3x3x_als_i2c_client, 1)))		//changed by micheal for als ps compatible 20190322
			{
		...
		{
			if ((err = stk3x3x_enable_als(stk3x3x_als_i2c_client, 0)))		//changed by micheal for als ps compatible 20190322
			{
	...
	case ALSPS_GET_ALS_DATA:
		...
		dat = stk3x3x_get_als_value(obj, obj->als);
...
static int stk3x3x_i2c_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	/*
		if(msg.event == PM_EVENT_SUSPEND)
		{
			...
			if((err = stk3x3x_enable_als(client, 0)))
			{
			...
			if((err = stk3x3x_enable_ps(client, 0, 1)))
			{
				APS_ERR("disable ps:  %d\n", err);
				return err;
			}

			stk3x3x_power(obj->hw, 0);
		}
...
static int stk3x3x_i2c_resume(struct i2c_client *client)
{
	...
	/*
		...
		{
			if((err = stk3x3x_enable_als(client, 1)))
			{
		...
		{
			if((err = stk3x3x_enable_ps(client, 1, 1)))
			{
...
static int als_enable_nodata(int en)
{
	...
	APS_LOG("stk3x3x_obj als enable value = %d\n", en);
#ifdef CUSTOM_KERNEL_SENSORHUB
	...
	res = stk3x3x_enable_als(stk3x3x_als_i2c_client, en);		//changed by micheal for als ps compatible 20190322
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
...
static int ps_enable_nodata(int en)
{
	...
	int len;
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
	APS_LOG("stk3x3x_obj ps enable value = %d\n", en);
#ifdef CUSTOM_KERNEL_SENSORHUB
...
static int stk3x3x_init_reg(struct i2c_client *client)
{
	...
	//client->addr = 0x47;		//delete by michela for als ps compatible 20190322
	printk("client->addr======%x\n", client->addr);
	//obj->client = client;		//delete by michela for als ps compatible 20190322
	...
	i2c_config_reg = STK_PS_PRS4 | STK_PS_GAIN8 | STK_PS_IT400;
	atomic_set(&obj->psctrl_val, i2c_config_reg);

	i2c_config_reg = STK_ALS_PRS1 | STK_ALS_GAIN64 | STK_ALS_IT50;
	atomic_set(&obj->alsctrl_val, i2c_config_reg);

	i2c_config_reg = STK_ALSC_GAIN16;
	atomic_set(&obj->alsctrl2_val, i2c_config_reg);

	obj->ledctrl_val = STK_LED_12_5mA;
	obj->wait_val = STK_WAIT50;
	obj->int_val = STK_INT_PS_MODE1;
	obj->intell_val = STK_INTELL_20;
	...
	//reflesh einit when threshold set
	i2c_config_reg = 0x01;
	ret = stk3x3x_master_send(client, 0xFA, &i2c_config_reg, 1);
...
static int stk3x3x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	stk3x3x_obj = obj;
	obj->client = client;		//add by micheal for ps als compatible 20190322
	//obj->hw = hw;
	stk3x3x_get_addr(&obj->addr);
	INIT_DELAYED_WORK(&obj->eint_work, stk3x3x_eint_work);
	stk3x3x_init_reg(client);
	stk3x3x_init_reg(stk3x3x_als_i2c_client);	//add by micheal for als init 20190322
...
//add by micheal for als init start 20190322
	if ((err = stk3x3x_init_als_client(stk3x3x_als_i2c_client)))
	{
		printk("stk3x3x_als_i2c_client init failed!\n");
		goto exit_init_failed;
	}
//add by micheal for als init end 2019.322
	//alsps_name = "stk3x3x";


//hct-drv bob.chen add for Discrete ALSPS begin. no use this attr, we use DiscreteALSPS attr.
#if defined(CONFIG_DISCRETEALSPS)
        if(true){
            stk3x3x_init_flag = 0;
            APS_LOG("%s: OK, return it in earlier time, not to set ATTR. \n", __FUNCTION__);
            DiscreteALSPS_init_done(DISCRETEALSPS_PS, 1);

            return 0;
        }
#endif
//hct-drv bob.chen add for Discrete ALSPS end.
	...
	if ((err = stk3x3x_create_attr(&(stk3x3x_init_info.platform_diver_addr->driver))))
		//if((err = stk3x3x_create_attr(&stk3x3x_alsps_driver.driver)))
	{
...
//hct-drv bob.chen add for Discrete ALSPS begin.
#if defined(CONFIG_DISCRETEALSPS)
DISCRETEALSPS_DIRVER stk3x3x_func = 
{
	.name		= "stk3x3x_func",
        .unlocked_ioctl     = stk3x3x_unlocked_ioctl, 

        .als_open_report_data     = als_open_report_data, 
        .als_enable_nodata     = als_enable_nodata, 
        .als_set_delay     = als_set_delay, 
        .als_batch     = als_batch, 
        .als_flush     = als_flush, 
        .als_get_data     = als_get_data, 
        
        .ps_open_report_data     = ps_open_report_data, 
        .ps_enable_nodata     = ps_enable_nodata, 
        .ps_set_delay     = ps_set_delay, 
        .ps_batch     = ps_batch, 
        .ps_flush     = ps_flush, 
        .ps_get_data     = ps_get_data, 

        .show_als     = stk3x3x_show_als, 
        .show_ps     = stk3x3x_show_ps, 
        .show_alslv     = stk3x3x_show_alslv, 
        .store_alslv     = stk3x3x_store_alslv, 
        .show_alsval     = stk3x3x_show_alsval, 
        .store_alsval     = stk3x3x_store_alsval, 
        .show_cali     = stk3x3x_show_cali, 
        .ps_maxdiff_store     = stk3x3x_ps_maxdiff_store, 
        .ps_maxdiff_show     = stk3x3x_ps_maxdiff_show, 
        .ps_ltnct_store     = stk3x3x_ps_ltnct_store, 
        .ps_ltnct_show     = stk3x3x_ps_ltnct_show, 
        .ps_htnct_store     = stk3x3x_ps_htnct_store, 
        .ps_htnct_show     = stk3x3x_ps_htnct_show, 

//        .show_firlen     = stk3x3x_show_firlen, 
//        .store_firlen     = stk3x3x_store_firlen, 
 //       .show_ir     = stk3x3x_show_ir, 
        .store_trace     = stk3x3x_store_trace, 
        .show_trace     = stk3x3x_show_trace, 
        .store_config     = stk3x3x_store_config, 
        .show_config     = stk3x3x_show_config, 
        .show_reg     = stk3x3x_show_reg, 
        .show_send     = stk3x3x_show_send, 
        .store_send     = stk3x3x_store_send, 
        .show_recv     = stk3x3x_show_recv, 
        .store_recv     = stk3x3x_store_recv, 
        .show_allreg     = stk3x3x_show_allreg, 
        .show_status     = stk3x3x_show_status, 
...
{
	APS_FUN();
	printk("stk3x3x_init In");
	alsps_driver_add(&stk3x3x_init_info);// hwmsen_alsps_add(&stk3x3x_init_info);

//hct-drv bob.chen add for Discrete ALSPS begin.
#if defined(CONFIG_DISCRETEALSPS)
    DiscreteALSPS_add(DISCRETEALSPS_PS, &stk3x3x_func);
#endif
//hct-drv bob.chen add for Discrete ALSPS end.

#if 0
	stk3x3x_regulator = regulator_get(&alspsPltFmDev->dev, "vstk3x3x");

	if (!stk3x3x_regulator)
		printk("stk3x3x_regulator is NULL\n");

	ret = regulator_set_voltage(stk3x3x_regulator, 2800000, 2800000); /*set 2.8v*/
