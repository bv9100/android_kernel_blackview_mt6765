...
//hct-drv bob.chen modify for Discrete ALSPS begin. set as only als, not use ps.
...
struct alsps_hw stk3x1x_alsps_cust;
static struct alsps_hw *hw = &stk3x1x_alsps_cust;
struct platform_device *alspsPltFmDev_stk3x1x;
/* For alsp driver get cust info */
#if 0
static struct alsps_hw *get_cust_alsps(void)
{
	return &stk3x1x_alsps_cust;
}
#endif
/*----------------------------------------------------------------------------*/
typedef enum {
...
int stk3x1x_read_id(struct i2c_client *client)
{
	...
	if (buf[0] == 0x12)
	{
		atomic_set(&obj->alsctrl_val, 0x29);
	}
...
#ifdef STK_TUNE0	 //hct-drv add build pass.
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
	...
	alspsPltFmDev_stk3x1x = get_alsps_platformdev();
/* gpio setting */

//hct-drv bob.chen modify for Discrete ALSPS begin. set as only als, not use ps. need return it to void register the eint.
    #if defined(CONFIG_DISCRETEALSPS)
        if(true){
            return 0;
        }
...
//hct-drv bob.chen modify for Discrete ALSPS end. 
	...
	pinctrl = devm_pinctrl_get(&alspsPltFmDev_stk3x1x->dev);
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
...
/* eint request */
	if (stk3x1x_obj->irq_node) {
        ...
        enable_irq_wake(stk3x1x_obj->irq); //@20170707 modify by mtk for fix bug(deep sleep cannot resume)
	} else {
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		APS_ERR("stk3x1x_i2c_probe. get dts info fail\n");
...
//hct-drv bob.chen add for Discrete ALSPS begin. no use this attr, we use DiscreteALSPS attr.
    ...
    if(true){
            ...
            DiscreteALSPS_init_done(DISCRETEALSPS_ALS, 1);
            return 0;
        }
#endif
...
#if defined(CONFIG_DISCRETEALSPS)
DISCRETEALSPS_DIRVER stk3x1x_func = 
{
	.name		= "stk3x1x_func",
        .unlocked_ioctl     = stk3x1x_unlocked_ioctl, 
        ...
        .show_als     = stk3x1x_show_als, 
        .show_ps     = stk3x1x_show_ps, 
        .show_alslv     = stk3x1x_show_alslv, 
        .store_alslv     = stk3x1x_store_alslv, 
        .show_alsval     = stk3x1x_show_alsval, 
        .store_alsval     = stk3x1x_store_alsval, 

#ifdef STK_TUNE0
    .show_cali     = stk3x1x_show_cali, 
    .ps_maxdiff_store     = stk3x1x_ps_maxdiff_store, 
    .ps_maxdiff_show     = stk3x1x_ps_maxdiff_show, 
    .ps_ltnct_store     = stk3x1x_ps_ltnct_store, 
    .ps_ltnct_show     = stk3x1x_ps_ltnct_show, 
    .ps_htnct_store     = stk3x1x_ps_htnct_store, 
    .ps_htnct_show     = stk3x1x_ps_htnct_show, 
#endif


        .show_firlen     = stk3x1x_show_firlen, 
        .store_firlen     = stk3x1x_store_firlen, 
        .show_ir     = stk3x1x_show_ir, 
        .store_trace     = stk3x1x_store_trace, 
        .show_trace     = stk3x1x_show_trace, 
        .store_config     = stk3x1x_store_config, 
        .show_config     = stk3x1x_show_config, 
        .show_reg     = stk3x1x_show_reg, 
        .show_send     = stk3x1x_show_send, 
        .store_send     = stk3x1x_store_send, 
        .show_recv     = stk3x1x_show_recv, 
        .store_recv     = stk3x1x_store_recv, 
        .show_allreg     = stk3x1x_show_allreg, 
        .show_status     = stk3x1x_show_status, 
...
{
#if 0
	const char *name = "mediatek,cust_stk3x1x";
...
//hct-drv bob.chen add for Discrete ALSPS begin.
#if defined(CONFIG_DISCRETEALSPS)
    DiscreteALSPS_add(DISCRETEALSPS_ALS, &stk3x1x_func);
#endif
//hct-drv bob.chen add for Discrete ALSPS end.
