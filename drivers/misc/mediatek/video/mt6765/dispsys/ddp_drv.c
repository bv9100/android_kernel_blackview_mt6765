...
#ifdef __HCT_MT6370_LCM_VOLTAGE_58__
...
static struct pinctrl_state  *lcm_rest_en0, * lcm_rest_en1, *lcm_power_dm_en0,*lcm_power_dm_en1,
...
int mt_dsi_get_gpio_info(struct platform_device *pdev)
{
	...
	lcm_rest_en0 = pinctrl_lookup_state(dsi_pinctrl1, "lcm_rst_out0_gpio");
	if (IS_ERR(lcm_rest_en0)) {
		ret = PTR_ERR(lcm_rest_en0);
		dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcm_rest_en0!\n");
	}
	lcm_rest_en1 = pinctrl_lookup_state(dsi_pinctrl1, "lcm_rst_out1_gpio");
	if (IS_ERR(lcm_rest_en1)) {
		ret = PTR_ERR(lcm_rest_en1);
		dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcm_rst_out1_gpio!\n");
	}
      ...
      lcm_power_enn0 = pinctrl_lookup_state(dsi_pinctrl1, "lcd_bias_enn0_gpio");
      if (IS_ERR(lcm_power_enn0)) {
          ret = PTR_ERR(lcm_power_enn0);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcd_bias_enn0_gpio!\n");
      }
      lcm_power_enn1 = pinctrl_lookup_state(dsi_pinctrl1, "lcd_bias_enn1_gpio");
      if (IS_ERR(lcm_power_enn1)) {
          ret = PTR_ERR(lcm_power_enn1);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcd_bias_enn1_gpio!\n");
      }
      lcm_power_enp0 = pinctrl_lookup_state(dsi_pinctrl1, "lcd_bias_enp0_gpio");
      if (IS_ERR(lcm_power_enp0)) {
          ret = PTR_ERR(lcm_power_enp0);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcd_bias_enp0_gpio!\n");
      }
      lcm_power_enp1 = pinctrl_lookup_state(dsi_pinctrl1, "lcd_bias_enp1_gpio");
      if (IS_ERR(lcm_power_enp1)) {
          ret = PTR_ERR(lcm_power_enp1);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcd_bias_enp1_gpio!\n");
      }
      tp_incell_rst0_gpio = pinctrl_lookup_state(dsi_pinctrl1, "tp_incell_rst0_gpio");
      if (IS_ERR(tp_incell_rst0_gpio)) {
          ret = PTR_ERR(tp_incell_rst0_gpio);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl tp_incell_rst0_gpio!\n");
      }
      tp_incell_rst1_gpio = pinctrl_lookup_state(dsi_pinctrl1, "tp_incell_rst1_gpio");
      if (IS_ERR(tp_incell_rst1_gpio)) {
          ret = PTR_ERR(tp_incell_rst1_gpio);
          dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl tp_incell_rst1_gpio!\n");
      }
      return ret;
}
int mt_dsi_pinctrl_set(unsigned int pin , unsigned int level)
{
	...
	case LCM_POWER_DM_NO: 	
      ...
      case LCM_POWER_ENP:   
          if( IS_ERR(lcm_power_enp0)|| IS_ERR(lcm_power_enp1))
          {
           ...
           break;
      case TP_INCELL_RST:   
          if( IS_ERR(tp_incell_rst0_gpio)|| IS_ERR(tp_incell_rst1_gpio))
          {
               pr_err("err: lcm_power_enp0 or lcm_power_enp1 is error!!!\n");
               return ret;
          }   
	...
	default:
          {
              pr_err("err: leds_gpio_output pin[%d] is error!!, drv need to check!\n", pin);
              return -1;
           }
	}
	switch(pin)
	...
	case TP_INCELL_RST: pinctrl_select_state(dsi_pinctrl1, level ? tp_incell_rst1_gpio : tp_incell_rst0_gpio); break; 		
	default: ;
	}
      return 0;
}
