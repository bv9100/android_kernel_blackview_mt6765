...
//hct-drv add for __HCT_LCM_RST_PIN_POWER_OFF_GPIO_M__ begin
#include <linux/hct_include/hct_project_all_config.h>
//hct-drv add for __HCT_LCM_RST_PIN_POWER_OFF_GPIO_M__ end
...
//hct-drv chenyichong add for LCM power begin.	
extern int RT5081_db_pos_neg_setting(void);
extern int RT5081_db_pos_neg_disable(void);
extern int RT5081_db_pos_neg_setting_by_i2c(int delay);
extern int RT5081_db_pos_neg_disable_by_i2c(int ms);
//hct-drv chenyichong add for LCM power end.
#endif
...
enum DSI_STATUS DSI_PS_Control(enum DISP_MODULE_ENUM module,
	struct cmdqRecStruct *cmdq, LCM_DSI_PARAMS *dsi_params, int w,
	int h)
{
	int i = 0;
...
long set_gpio_tp_incell_rst(unsigned int value)
{
	long ret = 0;

#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_TP_INCELL_RST1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_TP_INCELL_RST0);
#endif

	return ret;
}
//hct-drv add 20181018 end
...
void DSI_set_cmdq_V3_Wrapper_DSI0(LCM_setting_table_V3 *para_tbl,
...
void DSI_set_cmdq_V3_Wrapper_DSI1(LCM_setting_table_V3 *para_tbl,
	unsigned int size, unsigned char force_update)
{
...
void DSI_set_cmdq_V3_Wrapper_DSIDual(LCM_setting_table_V3 *para_tbl,
	unsigned int size, unsigned char force_update)
{
...
#ifdef CONFIG_MT6370_PMU_DSV
//hct-drv chenyichong add for LCM power begin.	
long hct_lcm_power_settings(HCT_LCM_POWER_MODE mode, unsigned int value, unsigned int mdelay1, unsigned int mdelay2)
{
	long ret = 0;

	switch (mode) {
	case HCT_LCM_POWER_MODE0_SYSTEM:
		DISPERR("[hct_lcm_power_settings] HCT_LCM_POWER_MODE0_SYSTEM. \n");
		//do nothing.
		break;
	case HCT_LCM_POWER_MODE1_RT5081:
		DISPERR("[hct_lcm_power_settings] HCT_LCM_POWER_MODE1_RT5081. \n");		
		if (value){
			RT5081_db_pos_neg_setting();
			lcm_mdelay(mdelay1);
			lcd_enp_bias_setting(1);//3.3V
			lcm_mdelay(mdelay2);
			lcd_enn_bias_setting(1);
		}
		else{
			lcd_enp_bias_setting(0);//3.3V
			lcm_mdelay(mdelay1);
		...
		break;
	case HCT_LCM_POWER_MODE1_RT5081_BY_I2C:
		DISPERR("[hct_lcm_power_settings] HCT_LCM_POWER_MODE1_RT5081_BY_I2C. \n");		
		if (value){
			RT5081_db_pos_neg_setting_by_i2c(mdelay1);
			lcm_mdelay(mdelay2);
		}
		else{
			//RT5081_db_pos_neg_disable();
			RT5081_db_pos_neg_disable_by_i2c(mdelay1);
		}
		break;
	case HCT_LCM_POWER_MODE2_V1_8:
		DISPERR("[hct_lcm_power_settings] HCT_LCM_POWER_MODE2_V1_8. \n");
		if (value){
		...
		break;
	case HCT_LCM_POWER_MODE3_V2_8:
		DISPERR("[hct_lcm_power_settings] HCT_LCM_POWER_MODE3_V2_8. \n");
		if (value){
			lcd_enp_bias_setting(1);//3.3V
			lcm_mdelay(mdelay1);
			lcd_enn_bias_setting(1);
		...
		break;
	default:
		DISPERR("[hct_lcm_power_settings] default. \n");
		break;
	}
	...
	LCM_DRIVER *lcm_drv)
{
	...
	//hct-drv chenyichong add for LCM power begin.
#ifdef CONFIG_MT6370_PMU_DSV
	utils->hct_lcm_power_settings = hct_lcm_power_settings;
#endif
	//hct-drv chenyichong add for LCM power end.
	utils->set_gpio_tp_incell_rst = set_gpio_tp_incell_rst;
#endif
#endif
	...
	void *params)
{
	...
	LCM_DSI_MODE_SWITCH_CMD lcm_cmd =
		*((LCM_DSI_MODE_SWITCH_CMD *) (params));
...
int DSI_esd_check_num(LCM_DSI_PARAMS *dsi_params)
{
	int i, cnt;
...
#ifdef __HCT_FT8719_ESD_CHECK_BUF__
#if __HCT_FT8719_ESD_CHECK_BUF__
				if(i == 2)
				{
					buffer[j] &= 0x0f; 
					printk("buffer[%d]=0x%x,i=%d\n", j, buffer[j], i);
				}
#endif
#endif
