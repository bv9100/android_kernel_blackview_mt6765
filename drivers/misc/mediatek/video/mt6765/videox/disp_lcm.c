...
struct disp_lcm_handle *disp_lcm_probe(char *plcm_name,
	LCM_INTERFACE_ID lcm_id, int is_lcm_inited)
{
	int lcmindex = 0;
	...
	lcm_param = kzalloc(sizeof(uint8_t *) * sizeof(LCM_PARAMS),
		GFP_KERNEL);
	if (plcm && lcm_param) {
...
int disp_lcm_ioctl(struct disp_lcm_handle *plcm, LCM_IOCTL ioctl,
	unsigned int arg)
{
	return 0;
}
