...
int64_t hct_esd_lcm_resume_time = 0;
#endif
//hct-drv add end.
...
//hct-drv add begin, fix mt6761 lcm screen bluerred when screenon. this is a workaround solution.
#if defined(CONFIG_MACH_MT6761) 
        if(get_monotonic_coarse().tv_sec - hct_esd_lcm_resume_time < 5){
            primary_display_manual_unlock();
            _primary_path_switch_dst_unlock();
            continue;
        }
...
//hct-drv add end.
		...
		do {
			...
			if (!ret) /* success */
				break;
                              #if defined(__HCT_ESD_BUG_FOR_JD9366__)
                                        if(((i%2) == 0) && (i > 0))
                                        {
                                                primary_display_esd_recovery();
                                                DISPERR("[ESD]JD9366_esd check fail, will do esd recovery. try=%d\n", i);
                                                recovery_done = 1;
                                                msleep(200);
                                        }
                              #else
