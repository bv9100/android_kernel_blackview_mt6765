...
//hct-drv add begin, fix mt6761 lcm screen bluerred when screenon. this is a workaround solution.
#if defined(CONFIG_MACH_MT6761) 
extern  int64_t hct_esd_lcm_resume_time;
#endif
//hct-drv add end.
...
int primary_display_resume(void)
{
	...
	if (primary_display_get_power_mode_nolock() == DOZE) {
		...
		if (primary_display_get_lcm_power_state_nolock() != LCM_ON) {
			...
			//hct-drv modify begin, fix mt6761 lcm screen bluerred when screenon. this is a workaround solution.
            #if defined(CONFIG_MACH_MT6761)
                        hct_esd_lcm_resume_time = get_monotonic_coarse().tv_sec; //  + time.tv_nsec;
                        DISPCHECK("[POWER]lcm resume[end]. hct_esd_lcm_resume_time = %lld. \n", hct_esd_lcm_resume_time);
            #else
...
int primary_display_resolution_test(void)
{
	...
	int dst_width = 0;
	int dst_heigh = 0;
	LCM_DSI_MODE_CON dsi_mode_backup =
		primary_display_is_video_mode();
