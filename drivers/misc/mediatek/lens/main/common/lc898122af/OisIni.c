...
void	IniClk( void )
{
	...
	if( UcModule == MODULE_20M ){	//20M
		RegWriteA_LC898122AF( OPAFDIV,		0x06 ) ;		// 0x0214	48MHz/6 = 8MHz
	}
	else{							//13M
...
void	IniDgy( void )
{
	...
	RegWriteA_LC898122AF( GRINI	, 0x80 );					// 0x0281	[ PARA_REG | AXIS7EN | AXIS4EN | - ][ - | SLOWMODE | - | - ]
  	...
  	RegWriteA_LC898122AF( LSBF	, 0x03 );						// 0x028D

	/*Set Digital Gyro Settings*/
	RegWriteA_LC898122AF( GRADR0	, 0x0B );					// 0x0283
	RegWriteA_LC898122AF( GSETDT	, 0x0F );					// 0x028A	Enable BDU, Enable 3-wire SPI mode
	RegWriteA_LC898122AF( GRACC	, 0x10 );					// 0x0282	[ ADRPLUS(1:0) | - | WR1B ][ - | RD4B | RD2B | RD1B ]
	AccWit( 0x10 ) ;								/* Digital Gyro busy wait 				*/

	/*Set Digital Gyro Settings*/
	RegWriteA_LC898122AF( GRADR0	, 0x0D );					// 0x0283
	RegWriteA_LC898122AF( GSETDT	, 0x01 );					// 0x028A	LPF = 300Hz
	RegWriteA_LC898122AF( GRACC	, 0x10 );					// 0x0282	[ ADRPLUS(1:0) | - | WR1B ][ - | RD4B | RD2B | RD1B ]
	AccWit( 0x10 ) ;								/* Digital Gyro busy wait 				*/
	...
	RegWriteA_LC898122AF( GRINI, ( UcGrini | SLOWMODE) );		// 0x0281	[ PARA_REG | AXIS7EN | AXIS4EN | - ][ - | SLOWMODE | - | - ]
	...
	RegWriteA_LC898122AF( PANAM	, 0x09 );					// 0x028E	Panasonic mode, Burst Output
	RegWriteA_LC898122AF( REVB7	, 0x03 );					// 0x028C	Reverse bit7
 #endif // USE_PANASONIC
...
void	IniSrv( void )
{
	...
	RegWriteA_LC898122AF( WH_SMTTMR,	0x07 ) ;				// 0x017E		10ms=(0+1)*16/11.718kHz
#else	//USE_EXTCLK_ALL
	RegWriteA_LC898122AF( WH_SMTSRVSMP,0x06 ) ;				// 0x017D		2.7ms=2^06/23.4375kHz
	RegWriteA_LC898122AF( WH_SMTTMR,	0x0f ) ;				// 0x017E		10ms=(15+1)*16/23.4375kHz
	...
	RegWriteA_LC898122AF( PWMFC,		0x7C ) ;				// 0x0011	VREF, PWMFRQ=7:PWMCLK(EXCLK)/PWMPERIODX[5:2]=18MHz/4=4.5MHz, MODE0B, 11-bit Accuracy
	...
	if( UcCvrCod == CVER122 ) {
		...
		RegWriteA_LC898122AF( PWMFC,   0x1D ) ;				// 0x0011	VREF, PWMCLK/512, MODE0B, 12Bit Accuracy
	}
...
#ifdef	USE_VH_SYNC
	RegWriteA_LC898122AF( STROBEFC,	0x80 ) ;				// 0x001C	?O??????Strobe?M????L??
...
#else		//PWM_CAREER_TEST
	#ifdef	USE_EXTCLK_PWM
		RegWriteA_LC898122AF( PWMPERIODX,	0x84 ) ;			// 0x0018		PWM Carrier Freq
		RegWriteA_LC898122AF( PWMPERIODX2,	0x00 ) ;			// 0x0019		PWM Carrier Freq
		RegWriteA_LC898122AF( PWMPERIODY,	0x84 ) ;			// 0x001A		PWM Carrier Freq
		RegWriteA_LC898122AF( PWMPERIODY2,	0x00 ) ;			// 0x001B		PWM Carrier Freq
	#else	//USE_EXTCLK_PWM
...
void	IniGyr( void )
{
	...
	RegWriteA_LC898122AF( WG_LMT3MOD , 0x00 );						// 0x0118 	[ - | - | - | - ][ - | - | - | CmLmt3Mod ]
														//				CmLmt3Mod	0: ????~?b?^?[????, 1: ?~????a???~?b?^?[????
	RegWriteA_LC898122AF( WG_VREFADD , 0x12 );						// 0x0119	 	?Z???^?[??????s???x??RAM??A?h???X????6?r?b?g?@(default 0x12 = GXH1Z2/GYH1Z2)
#endif
	...
	RamWrite32A_LC898122AF( gylmt4HS0, GYRLMT4_S1 ) ;				// 0x112B	Y??Limiter4 High?l0
	RamWrite32A_LC898122AF( gxlmt4HS0, GYRLMT4_S1 ) ;				// 0x102B	X??Limiter4 High?l0

	RamWrite32A_LC898122AF( gxlmt4HS1, GYRLMT4_S2 ) ;				// 0x102C	X??Limiter4 High?l1
	RamWrite32A_LC898122AF( gylmt4HS1, GYRLMT4_S2 ) ;				// 0x112C	Y??Limiter4 High?l1
	...
	RamWrite32A_LC898122AF( npxlev8, UnGyrLmt.UlLngVal ) ;			// 0x109B
	RamWrite32A_LC898122AF( npylev8, UnGyrLmt.UlLngVal ) ;			// 0x119B


	// Fast
//	UnGyrLmt.SfFltVal	= 0.0076F ;						// St1
	UnGyrLmt.SfFltVal	= 0.02F ;						// St1
	RamWrite32A_LC898122AF( npxlev1, UnGyrLmt.UlLngVal ) ;			// 0x100F
	RamWrite32A_LC898122AF( npylev1, UnGyrLmt.UlLngVal ) ;			// 0x110F
	RamWrite32A_LC898122AF( npxlev1_i, UnGyrLmt.UlLngVal ) ;		// 0x10CF
	RamWrite32A_LC898122AF( npylev1_i, UnGyrLmt.UlLngVal ) ;		// 0x11CF
	UnGyrLmt.SfFltVal	= 0.0005F ;						// St2 Limiter
	RamWrite32A_LC898122AF( npxlev2, UnGyrLmt.UlLngVal ) ;			// 0x101F
	RamWrite32A_LC898122AF( npylev2, UnGyrLmt.UlLngVal ) ;			// 0x111F
	RamWrite32A_LC898122AF( npxlev2_i, UnGyrLmt.UlLngVal ) ;		// 0x10DF
	RamWrite32A_LC898122AF( npylev2_i, UnGyrLmt.UlLngVal ) ;		// 0x11DF

	//Slow
	UnGyrLmt.SfFltVal	= 0.0005F ;  					// St3??
//	UnGyrLmt.SfFltVal	= 0.0050F ;  					// St3??
	RamWrite32A_LC898122AF( npxlev3, UnGyrLmt.UlLngVal ) ;			// 0x102F
	RamWrite32A_LC898122AF( npylev3, UnGyrLmt.UlLngVal ) ;			// 0x112F
//	UnGyrLmt.SfFltVal	= 0.001F ;  					// St3
	UnGyrLmt.SfFltVal	= 0.0025F ;  					// St3??
	RamWrite32A_LC898122AF( npxlev3_i, UnGyrLmt.UlLngVal ) ;		// 0x10EF
	RamWrite32A_LC898122AF( npylev3_i, UnGyrLmt.UlLngVal ) ;		// 0x11EF
//	UnGyrLmt.SfFltVal	= 0.0009F ;						// St4 Limiter
	UnGyrLmt.SfFltVal	= 0.0005f ;						// St4 Limiter
	RamWrite32A_LC898122AF( npxlev4, UnGyrLmt.UlLngVal ) ;			// 0x103F
	RamWrite32A_LC898122AF( npylev4, UnGyrLmt.UlLngVal ) ;			// 0x113F
//	UnGyrLmt.SfFltVal	= 0.0009F ;						// St4 Limiter(initial)
	UnGyrLmt.SfFltVal	= 0.0005F ;						// St4 Limiter
	RamWrite32A_LC898122AF( npxlev4_i, UnGyrLmt.UlLngVal ) ;		// 0x10FF
	RamWrite32A_LC898122AF( npylev4_i, UnGyrLmt.UlLngVal ) ;		// 0x11FF


//	RegWriteA_LC898122AF( WG_VREFADD , 0x12 );						// 0x0119

	// Pan/Tilt NEW?@PanTilt?@Setteing
	RegWriteA_LC898122AF( WG_NPANST12BTMR, 0x0F ) ;				// 0x0167
	RegWriteA_LC898122AF( WG_NPANST3RTMR, 0x0A ) ;					// 0x0166
	RegWriteA_LC898122AF( WG_NPANST12TMRX, 0x00 ) ;				// 0x0168 682u:682??
	RegWriteA_LC898122AF( WG_NPANST12TMRY, 0x00 ) ;				// 0x0169 682u:682??
	RegWriteA_LC898122AF( WG_NPANST3TMRX, 0x08 ) ;					// 0x016A 98ms
	RegWriteA_LC898122AF( WG_NPANST3TMRY, 0x08 ) ;					// 0x016B 98ms
	RegWriteA_LC898122AF( WG_NPANST4TMRX, 0x02 ) ;					// 0x016C 21ms
	RegWriteA_LC898122AF( WG_NPANST4TMRY, 0x02 ) ;					// 0x016D 21ms

	RegWriteA_LC898122AF( WG_NPANSTFRC, 0x00 ) ;					// 0x010B State posituon ON
	RegWriteA_LC898122AF( WG_NPANFUN, 0x01 ) ;						// 0x016E Gain Cut,decrease
	RegWriteA_LC898122AF( WG_NPANINITMR, 0x02 ) ;					// 0x016F 21ms '?]??????????????????l
	RegWriteA_LC898122AF( WG_NPANSTOFF, 0xA0 ) ;					// 0x010E State5,7 OFF

	RegWriteA_LC898122AF( WG_NPANTST0 ,0x08 );						// 0x0164 Option Setting

	UnGyrLmt.UlLngVal = 0x3951B717 ;					// St1 		decrease
	RamWrite32A_LC898122AF( gxistp_1u, UnGyrLmt.UlLngVal ) ;		// 0x1085
	RamWrite32A_LC898122AF( gyistp_1u, UnGyrLmt.UlLngVal ) ;		// 0x1185
	UnGyrLmt.UlLngVal = 0x3F7FFE80 ;
	RamWrite32A_LC898122AF( gxistp_2d, UnGyrLmt.UlLngVal );		// 0x1087	ST1,ST3?@Ccof Val
	RamWrite32A_LC898122AF( gyistp_2d, UnGyrLmt.UlLngVal );		// 0x1187	ST1,ST3?@Ccof Val
	UnGyrLmt.UlLngVal = 0x3F7FFE80 ;
	RamWrite32A_LC898122AF( gyistp_2u, UnGyrLmt.UlLngVal );		// 0x1188	ST2,ST4?@Ccof Val
	RamWrite32A_LC898122AF( gxistp_2u, UnGyrLmt.UlLngVal );		// 0x1088	ST2,ST4?@Ccof Val
...
void	IniFil( void )
{
	...
	RegWriteA_LC898122AF( WC_RAMACCXY, 	0x01 ) ;		// 0x018D	Simultaneously Setting On
	...
	RegWriteA_LC898122AF( WC_RAMACCXY,		0x00 ) ;		// 0x018D	Simultaneously Setting Off
#else	//INIT_FAST
	...
	{
		RegWriteA_LC898122AF( pFilReg[ UsAryId ].UsRegAdd, pFilReg[ UsAryId ].UcRegDat ) ;
		UsAryId++ ;
	}
	...
	{
		RamWrite32A_LC898122AF( pFilRam[ UsAryId ].UsRamAdd, pFilRam[ UsAryId ].UlRamDat ) ;
		UsAryId++ ;
	}
...
void	IniAdj( void )
{
	...
	/* OIS */
	RegWriteA_LC898122AF( CMSDAC0, BIAS_CUR_OIS ) ;		// 0x0251	Hall Dac?d??
	...
	/* AF */
	RegWriteA_LC898122AF( CMSDAC1, BIAS_CUR_AF ) ;			// 0x0252	Hall Dac?d??
...
//	UlWitCyc	= ( unsigned long )( ( float )UsWitTim / NOP_TIME / ( float )12 ) ;
	UlWitCyc	= ( unsigned long )( UsWitTim / NOP_TIME / 12 ) ;
...
void	SelectGySleep( unsigned char UcSelMode )
{
	...
	else
	{
		...
		RegReadA_LC898122AF( GRINI	, &UcGrini );					// 0x0281	[ PARA_REG | AXIS7EN | AXIS4EN | - ][ LSBF | SLOWMODE | I2CMODE | - ]
		RegWriteA_LC898122AF( GRINI, ( UcGrini & ~SLOWMODE) );		// 0x0281	[ PARA_REG | AXIS7EN | AXIS4EN | - ][ LSBF | SLOWMODE | I2CMODE | - ]
...
void	ClrGyr( unsigned short UsClrFil , unsigned char UcClrMod )
{
	...
	/*Enable Clear*/
	RegWriteA_LC898122AF( WC_RAMINITON	, UcClrMod ) ;	// 0x0102	[ - | - | - | - ][ - | - | ?x??Clr | ?W??Clr ]
...
//	#ifdef	AF_MID_MOUNT
		if( UcAfType == BI_DIR ){
			RegWriteA_LC898122AF( DRVFCAF,		0x10 ) ;		// DRVFCAF(0x0081)
		}
//	#else	//AF_MID_MOUNT
		else{
		  #ifdef	AF_PWMMODE
...
//#ifdef	AF_MID_MOUNT
	if( UcAfType == BI_DIR ){
		...
		RegWriteA_LC898122AF( DRVFCAF,		0x10 ) ;			// DRVFCAF(0x0081)
		RegWriteA_LC898122AF( DRVFC3AF,	0x40 ) ;			// DRVFC3AF(0x0083)
		RegWriteA_LC898122AF( DRVFC4AF,	0x80 ) ;			// DRVFC4AF(0x0084)
		RegWriteA_LC898122AF( AFFC,		0x90 ) ;			// AFFC(0x0088)
		//RegWriteA_LC898122AF( CCAAF,		0x80 ) ;			// CCAAF(0x00A0)
	}
//#else	//AF_MID_MOUNT
	else{
	  #ifdef	AF_PWMMODE
...
//#ifdef	AF_MID_MOUNT
	if( UcAfType == BI_DIR ){
		RegWriteA_LC898122AF( CCFCAF,	 0x08 ) ;					// CCFCAF(0x00A1)
	}
//#else	//AF_MID_MOUNT
	else{
...
//#endif	//AF_MID_MOUNT
	...
	RegWriteA_LC898122AF( TCODEH,		0x04 ) ;			// Fast Mode
...
//#ifdef	AF_MID_MOUNT
	if( UcAfType == BI_DIR ){
		RamWriteA_LC898122AF( TREG_H,	(UsTregAf << 5) ) ;		// TREG_H(0x0380) - TREG_L(0x0381)
													// TREG[15:5] 11bit
...
//#else	//AF_MID_MOUNT
		...
		RamWriteA_LC898122AF( TREG_H,	(UsTregAf << 6) ) ;		// TREG_H(0x0380) - TREG_L(0x0381)
													// TREG[15:4] 10bit
...
void	ChkCvr( void )
{
	...
	RegWriteA_LC898122AF( VRREG ,	(unsigned char)FW_VER );		// 0x2D0	Version
}
...
void	RemOff( unsigned char UcMod )
{
	switch ( UcMod ) {
		case OFF :
			RegWriteA_LC898122AF( WG_PANSTT6, 0x11 ) ;			// 0x010A	Pan/Tilt State0 setting
			RegWriteA_LC898122AF( WC_RAMACCXY, 	0x01 ) ;	// 0x018D	Simultaneously Setting On
			RamWrite32A_LC898122AF( gxia_1, 0x39D2BD40 ) ;		// 0x1043	3Hz
			RamWrite32A_LC898122AF( gxib_1, 0x39D2BD40 ) ;		// 0x1044
			RamWrite32A_LC898122AF( gxic_1, 0x3F7FCB40 ) ;		// 0x1045
			RegWriteA_LC898122AF( WC_RAMACCXY,		0x00 ) ;	// 0x018D	Simultaneously Setting Off
			RegWriteA_LC898122AF( WG_PANSTT6, 0x00 ) ;			// 0x010A	Pan/Tilt Normal operation
			break ;
		case ON :
			ClrGyr( 0x007F, CLR_FRAM1 ) ;			// 			Clear Gyro Filter
			RegWriteA_LC898122AF( WC_RAMACCXY, 	0x01 ) ;	// 0x018D	Simultaneously Setting On
			RamWrite32A_LC898122AF( gxia_1, 0x3aaf73c0 ) ;		// 0x1043	10Hz
			RamWrite32A_LC898122AF( gxib_1, 0x3aaf73c0 ) ;		// 0x1044
			RamWrite32A_LC898122AF( gxic_1, 0x3F7F5080 ) ;		// 0x1045
			RegWriteA_LC898122AF( WC_RAMACCXY,		0x00 ) ;	// 0x018D	Simultaneously Setting Off
			SetPanTiltMode( ON ) ; 					// Pan/Tilt control enable
			RegWriteA_LC898122AF( WG_PANSTT6, 0x44 ) ;			// 0x010A	Pan/Tilt State3 setting
			break ;
	}
}
