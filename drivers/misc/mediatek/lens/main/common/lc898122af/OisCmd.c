...
void	MesFil( unsigned char	UcMesMod )
{
	...
	if( !UcMesMod ) {								// Hall Bias&Offset Adjust
		...
		RamWrite32A_LC898122AF( mes1ad, 0x00000000 ) ;		// 0x10F3
		RamWrite32A_LC898122AF( mes1ae, 0x00000000 ) ;		// 0x10F4
		RamWrite32A_LC898122AF( mes1ba, 0x3F7FD400 ) ;		// 0x10F5	HPF5Hz
		RamWrite32A_LC898122AF( mes1bb, 0xBF7FD400 ) ;		// 0x10F6
		RamWrite32A_LC898122AF( mes1bc, 0x3F7FA840 ) ;		// 0x10F7
		RamWrite32A_LC898122AF( mes1bd, 0x00000000 ) ;		// 0x10F8
		RamWrite32A_LC898122AF( mes1be, 0x00000000 ) ;		// 0x10F9
		...
		RamWrite32A_LC898122AF( mes2ad, 0x00000000 ) ;		// 0x11F3
		RamWrite32A_LC898122AF( mes2ae, 0x00000000 ) ;		// 0x11F4
		RamWrite32A_LC898122AF( mes2ba, 0x3F7FD400 ) ;		// 0x11F5	HPF5Hz
		RamWrite32A_LC898122AF( mes2bb, 0xBF7FD400 ) ;		// 0x11F6
		RamWrite32A_LC898122AF( mes2bc, 0x3F7FA840 ) ;		// 0x11F7
		RamWrite32A_LC898122AF( mes2bd, 0x00000000 ) ;		// 0x11F8
		RamWrite32A_LC898122AF( mes2be, 0x00000000 ) ;		// 0x11F9
...
void	LopSin( unsigned char	UcDirSel, unsigned char	UcSonOff )
{
	...
	if( UcSonOff ) {
		...
		if( !UcDirSel ) {
			...
			UcEqSwY &= ~EQSINSW ;
			if( UcModule == MODULE_20M ){	//20M
				RamWrite32A_LC898122AF( sxsin, 0x3C676E00 ) ;			// 0x10D5		-37dB
			}
			else{							//13M
				RamWrite32A_LC898122AF( sxsin, 0x3BB84480 ) ;			// 0x10D5		-45dB
	  		}
		} else {
			...
			UcEqSwY |= 0x10 ;
			if( UcModule == MODULE_20M ){	//20M
				RamWrite32A_LC898122AF( sysin, 0x3C676E00 ) ;			// 0x11D5		-37dB
			}
			else{							//13M
				RamWrite32A_LC898122AF( sysin, 0x3BB84480 ) ;			// 0x10D5		-45dB
	  		}
		}
...
unsigned char	TneGvc( void )
{
	...
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGxoVal = (unsigned short)GenMes( AD2Z, 0 );		// 64???????l????	GYRMON1(0x1110) <- GXADZ(0x144A)
	RegWriteA_LC898122AF( IZAH, (unsigned char)(StAdjPar.StGvcOff.UsGxoVal >> 8) ) ;	// 0x02A0		Set Offset High byte
	RegWriteA_LC898122AF( IZAL, (unsigned char)(StAdjPar.StGvcOff.UsGxoVal) ) ;		// 0x02A1		Set Offset Low byte
	//////////
	// Y
	//////////
	RegWriteA_LC898122AF( WC_MES1ADD0, 0x00 ) ;		// 0x0194
	RegWriteA_LC898122AF( WC_MES1ADD1, 0x00 ) ;		// 0x0195
	ClrGyr( 0x1000 , CLR_FRAM1 );					// Measure Filter RAM Clear
	StAdjPar.StGvcOff.UsGyoVal = (unsigned short)GenMes( AD3Z, 0 );		// 64???????l????	GYRMON2(0x1111) <- GYADZ(0x14CA)
	RegWriteA_LC898122AF( IZBH, (unsigned char)(StAdjPar.StGvcOff.UsGyoVal >> 8) ) ;	// 0x02A2		Set Offset High byte
	RegWriteA_LC898122AF( IZBL, (unsigned char)(StAdjPar.StGvcOff.UsGyoVal) ) ;		// 0x02A3		Set Offset Low byte
...
void	SetSinWavePara( unsigned char UcTableVal ,  unsigned char UcMethodVal )
{
	...
	else
	{
		...
		if(( UcMethodVal != XHALWAVE ) && ( UcMethodVal != YHALWAVE )) {
#ifdef	USE_SINLPF
			RegWriteA_LC898122AF( WC_DPI1ADD0,  ( unsigned char )MES1BZ2 ) ;						/* 0x01B0	input Meas-Fil */
			RegWriteA_LC898122AF( WC_DPI1ADD1,  ( unsigned char )(( MES1BZ2 >> 8 ) & 0x0001 ) ) ;	/* 0x01B1	input Meas-Fil */
			RegWriteA_LC898122AF( WC_DPI2ADD0,  ( unsigned char )MES2BZ2 ) ;						/* 0x01B2	input Meas-Fil */
			RegWriteA_LC898122AF( WC_DPI2ADD1,  ( unsigned char )(( MES2BZ2 >> 8 ) & 0x0001 ) ) ;	/* 0x01B3	input Meas-Fil */
			RegWriteA_LC898122AF( WC_DPO1ADD0, ( unsigned char )SXOFFZ1 ) ;						/* 0x01B8	output SXOFFZ1 */
			RegWriteA_LC898122AF( WC_DPO1ADD1, ( unsigned char )(( SXOFFZ1 >> 8 ) & 0x0001 ) ) ;	/* 0x01B9	output SXOFFZ1 */
			RegWriteA_LC898122AF( WC_DPO2ADD0, ( unsigned char )SYOFFZ1 ) ;						/* 0x01BA	output SYOFFZ1 */
			RegWriteA_LC898122AF( WC_DPO2ADD1, ( unsigned char )(( SYOFFZ1 >> 8 ) & 0x0001 ) ) ;	/* 0x01BA	output SYOFFZ1 */
...
#endif
		RegWriteA_LC898122AF( STBB0 	, 0x00 );		// 0x0250 	[ STBAFDRV | STBOISDRV | STBOPAAF | STBOPAY ][ STBOPAX | STBDACI | STBDACV | STBADC ]
		RegWriteA_LC898122AF( STBB1 	, 0x00 );		// 0x0264 	[ - | - | - | - ][ - | STBAFOP1 | - | STBAFDAC ]
...
//		RegWriteA_LC898122AF( DACMONFC, 0x81 ) ;		// 0x0032	DAC Monitor Active
		...
		RegWriteA_LC898122AF( STBB0	, 0xDF );		// 0x0250 	[ STBAFDRV | STBOISDRV | STBOPAAF | STBOPAY ][ STBOPAX | STBDACI | STBDACV | STBADC ]
		break ;
	case STB2_ON:
#ifdef	AF_PWMMODE
#else
...
//		RegWriteA_LC898122AF( DACMONFC, 0x01 ) ;		// 0x0032	DAC Monitor Standby
		...
		break ;
	case STB2_OFF:
		RegWriteA_LC898122AF( CLKON,	0x1F ) ;		// 0x020B	[ - | - | CmOpafClkOn | CmAfpwmClkOn | CMGifClkOn  | CmScmClkOn  | CmSrvClkOn  | CmPwmClkOn  ]
		SelectGySleep( OFF ) ;				/* Gyro Wake Up */
//		RegWriteA_LC898122AF( DACMONFC, 0x81 ) ;		// 0x0032	DAC Monitor Active
		RegWriteA_LC898122AF( PWMMONA, 0x80 ) ;		/* 0x0030	Monitor Active	*/
		DrvSw( ON ) ;						/* Driver Mode setting */
...
//		RegWriteA_LC898122AF( DACMONFC, 0x01 ) ;		// 0x0032	DAC Monitor Standby
	...
	case STB3_OFF:
		...
		RegWriteA_LC898122AF( CLKON,	0x1F ) ;		// 0x020B	[ - | - | - | - | CMGifClkOn  | CmScmClkOn  | CmSrvClkOn  | CmPwmClkOn  ]
		SelectGySleep( OFF ) ;				/* Gyro Wake Up */
//		RegWriteA_LC898122AF( DACMONFC, 0x81 ) ;		// 0x0032	DAC Monitor Active
		RegWriteA_LC898122AF( PWMMONA, 0x80 ) ;		/* 0x0030	Monitor Active	*/
		DrvSw( ON ) ;						/* Driver Mode setting */
...
//		RegWriteA_LC898122AF( DACMONFC, 0x01 ) ;		// 0x0032	DAC Monitor Standby
		...
		RegWriteA_LC898122AF( CLKON, 0x04 ) ;			/* 0x020B	Servo & PWM Clock OFF + D-Gyro I/F ON	*/
		break ;
	case STB4_OFF:
		RegWriteA_LC898122AF( CLKON,	0x1F ) ;		// 0x020B	[ - | - | - | - | CMGifClkOn  | CmScmClkOn  | CmSrvClkOn  | CmPwmClkOn  ]
...
//		RegWriteA_LC898122AF( DACMONFC, 0x81 ) ;		// 0x0032	DAC Monitor Active
		...
		/************** special mode ************/
	case STB2_OISON:
		RegReadA_LC898122AF( STBB0 	, &UcStbb0 );		// 0x0250 	[ STBAFDRV | STBOISDRV | STBOPAAF | STBOPAY ][ STBOPAX | STBDACI | STBDACV | STBADC ]
		UcStbb0 &= 0x80 ;
		RegWriteA_LC898122AF( STBB0 	, UcStbb0 );		// 0x0250 	[ STBAFDRV | STBOISDRV | STBOPAAF | STBOPAY ][ STBOPAX | STBDACI | STBDACV | STBADC ]
		RegWriteA_LC898122AF( PWMA 	, 0x00 );		// 0x0010		PWM Standby
		RegWriteA_LC898122AF( CVA,  0x00 ) ;			/* 0x0020	LINEAR PWM mode standby	*/
		DrvSw( OFF ) ;						/* Drvier Block Ena=0 */
#ifdef	MONITOR_OFF
...
//		RegWriteA_LC898122AF( DACMONFC, 0x01 ) ;		// 0x0032	DAC Monitor Standby
		SelectGySleep( ON ) ;				/* Gyro Sleep */
		RegReadA_LC898122AF( CLKON, &UcClkon ) ;		/* 0x020B	PWM Clock OFF + D-Gyro I/F OFF	SRVCLK can't OFF */
		UcClkon &= 0x1A ;
		RegWriteA_LC898122AF( CLKON, UcClkon ) ;		/* 0x020B	PWM Clock OFF + D-Gyro I/F OFF	SRVCLK can't OFF */
		break ;
	case STB2_OISOFF:
		RegReadA_LC898122AF( CLKON, &UcClkon ) ;		/* 0x020B	PWM Clock OFF + D-Gyro I/F ON  */
		UcClkon |= 0x05 ;
		RegWriteA_LC898122AF( CLKON,	UcClkon ) ;		// 0x020B	[ - | - | CmOpafClkOn | CmAfpwmClkOn | CMGifClkOn  | CmScmClkOn  | CmSrvClkOn  | CmPwmClkOn  ]
		SelectGySleep( OFF ) ;				/* Gyro Wake Up */
//		RegWriteA_LC898122AF( DACMONFC, 0x81 ) ;		// 0x0032	DAC Monitor Active
		RegWriteA_LC898122AF( PWMMONA, 0x80 ) ;		/* 0x0030	Monitor Active	*/
		DrvSw( ON ) ;						/* Driver Mode setting */
	...
	case STB2_AFON:
		...
		RegWriteA_LC898122AF( PWMMONA, 0x00 ) ;		// 0x0030	Monitor Standby
#endif
		RegReadA_LC898122AF( CLKON, &UcClkon ) ;		/* 0x020B	OPAF Clock OFF + AFPWM OFF	SRVCLK can't OFF	*/
		UcClkon &= 0x07 ;
		RegWriteA_LC898122AF( CLKON, UcClkon ) ;		/* 0x020B	OPAF Clock OFF + AFPWM OFF	SRVCLK can't OFF	*/
		break ;
	case STB2_AFOFF:
		RegReadA_LC898122AF( CLKON, &UcClkon ) ;		/* 0x020B	OPAF Clock ON + AFPWM ON  */
		UcClkon |= 0x18 ;
		RegWriteA_LC898122AF( CLKON,	UcClkon ) ;		// 0x020B	[ - | - | CmOpafClkOn | CmAfpwmClkOn | CMGifClkOn  | CmScmClkOn  | CmSrvClkOn  | CmPwmClkOn  ]
		AfDrvSw( ON ) ;						/* AF Driver Mode setting */
		RegWriteA_LC898122AF( PWMAAF 	, 0x00 );		// 0x0090		AF PWM Standby
...
void	SetZsp( unsigned char	UcZoomStepDat )
{
	...
	if(UcZoomStepDat > (ZOOMTBL - 1))
		UcZoomStepDat = (ZOOMTBL -1) ;										/* ??????ZOOMTBL-1?????? */
...
void StbOnn( void )
{
	...
	if( (( UcRegValx & 0x01 ) != 0x01 ) && (( UcRegValy & 0x01 ) != 0x01 ))
	{
		...
		for (UcCnt = 0; UcCnt < 60; UcCnt++) {
			...
			WitTim_LC898122AF( WTLE );
		}

		RegWriteA_LC898122AF( WH_SMTSRVON,	0x00 ) ;				// 0x017C		Smooth Servo OFF

		if(UcCnt == 60){
//			RamWriteA_LC898122AF( SXOFFZ2   , 0 ) ;				// 0x1462		Offset 0 clear
//			RamWriteA_LC898122AF( SYOFFZ2   , 0 ) ;				// 0x14E2		Offset 0 clear
			RamWrite32A_LC898122AF ( SXOFFZ2, 0x00000000 ) ;		// 0x1462		Offset 0 clear
			RamWrite32A_LC898122AF ( SYOFFZ2, 0x00000000 ) ;		// 0x14E2		Offset 0 clear
		}
	}
	else
	{
...
unsigned short	OscAdjA( unsigned short UsTgtCnt )
{
	...
	RegReadA_LC898122AF( SRVDIV, &UcSrvDivBk ) ;			/* 0x0211 	Srv Div back UP */
	RegWriteA_LC898122AF( SRVDIV,	0x00 ) ;				// 0x0211	 SRV Clock = Xtalck
	RegWriteA_LC898122AF( OSCSET, OSCSET_INI ) ;			// 0x0257
 #ifdef	OSC_EXCLK
	RegReadA_LC898122AF( PWMDIV, &UcPwmDivBk ) ;			/* 0x0210 	Pwm Div back UP */
	RegReadA_LC898122AF( CLKSEL, &UcClkSelBk ) ;			/* 0x020C 	ClkSel back UP */
	RegWriteA_LC898122AF( PWMDIV, 0x00 ) ;					/* 0x0210 	Pwm Div 1/1 */
	RegWriteA_LC898122AF( CLKSEL, UcClkSelBk | 0x01 ) ;	/* 0x020C 	ClkSel re-store */
	UcOscMod = OSCMODE_EXC ;
 #else
	UcOscMod = OSCMODE_I2C ;
 #endif
	RegWriteA_LC898122AF( OSCMAMODE, UcOscMod ) ;								// 0x0265
	RegWriteA_LC898122AF( OSCTCNTL, (unsigned char)UsTgtCnt ) ;				// 0x0266
	RegWriteA_LC898122AF( OSCTCNTH, (unsigned char)( UsTgtCnt >> 8 ) ) ;		// 0x0267
	UcClkJdg = 0 ;

	while( UcClkJdg == 0 )
	{
		RegWriteA_LC898122AF( OSCCNTEN, 0x01 ) ;		// 0x0258	 Trigger ON
		RegWriteA_LC898122AF( OSCCNTEN, 0x00 ) ;		// 0x0258	 Trigger OFF
		RegReadA_LC898122AF(  OSCMAMODE, &UcClkJdg ) ;	// 0x0265	 Status Read
		UcClkJdg &= 0x03;
	}

	UsResult = EXE_END ;

	RegReadA_LC898122AF( OSCSET, &UcOscSetVal ) ;		// 0x0257
	StAdjPar.UcOscVal = UcOscSetVal ;
	RegWriteA_LC898122AF( OSCMAMODE, OSCMODE_DEF ) ;	// 0x0265
	...
	RegWriteA_LC898122AF( SRVDIV,	UcSrvDivBk ) ;		// 0x0211	 SRV Clock set
 #ifdef	OSC_EXCLK
	RegWriteA_LC898122AF( PWMDIV, UcPwmDivBk ) ;		/* 0x0210 	Pwm Div re-store */
	RegWriteA_LC898122AF( CLKSEL, UcClkSelBk ) ;		/* 0x020C 	ClkSel re-store */
 #endif
	return( UsResult );
}
...
#ifndef	AFMID_TEST
	RegReadA_LC898122AF( STBB0,	&ucSTBB0 );						// 0x0250
	RegReadA_LC898122AF( OPGSEL2,	&ucOPGSEL2 );					// 0x0255
	RegReadA_LC898122AF( VGA_SW1,	&ucVGA_SW1 );					// 0x025B
	RamRead32A_LC898122AF( DAZHLO, &ulDAZHLO );					// 0x1529
	ucDOFSTDAF = GetDOFSTDAF();							// DOFSTDAF 0x0083 0x0084
	RamReadA_LC898122AF( TREG_H,	&usTREG_HL ) ;					// TREG_H(0x0380) - TREG_L(0x0381)
#endif	//AFMID_TEST
...
////////// ////////// ////////// ////////// //////////
	...
	RegWriteA_LC898122AF( STBB0,		0x83 ) ;					// 0x0250	AF_Drv=ON, DAC_V=ON, ADC=ON
	RegWriteA_LC898122AF( VGA_SW1,		0x04 ) ;					// 0x025B	OP_SWAF3=off, OP_SWAF2=on, OP_SWAF1=off, OP_SWAF0=off
...
////////// ////////// ////////// ////////// //////////
	...
	RegWriteA_LC898122AF( STBB0,		0xA3 ) ;					// 0x0250	AF_Drv=ON, AF_Op=ON, DAC_V=ON, ADC=ON
	RegWriteA_LC898122AF( VGA_SW1,		0x04 ) ;					// 0x025B	OP_SWAF3=off, OP_SWAF2=on, OP_SWAF1=off, OP_SWAF0=off
	RegWriteA_LC898122AF( OPGSEL2,		0x00 ) ;					// 0x0255	x6
	RamWrite32A_LC898122AF( DAZHLO, 	UnMesOff.UlLngVal );		// 0x1529	VGAAF Offset
...
//			UnMesOff1.SfFltVal += 0.01f;
  if( UnMesOff2.SfFltVal < -1 ){
			UnMesOff1.SfFltVal += 1;
		}
		else{
			break;
		}
		RamWrite32A_LC898122AF( DAZHLO, 	UnMesOff1.UlLngVal );	// 0x1529	VGAAF Offset
...
////////// ////////// ////////// ////////// //////////
	...
	UnStep.SfFltVal = ( UnMesMin.SfFltVal - UnMesMax.SfFltVal ) / 31;
...
//	cAdjOft = (char)(UnAdjOft.SfFltVal + 0.5f);
  cAdjOft = (char)(UnAdjOft.SfFltVal);
...
////////// ////////// ////////// ////////// //////////
	...
	RegReadA_LC898122AF( DRVFC3AF, &ucDRVFC3AF );
	RegReadA_LC898122AF( DRVFC4AF, &ucDRVFC4AF );
...
//	if( UnAdjRes.SfFltVal > ( UnAdjMin.SfFltVal + 0.02 ) ){
  if( UnAdjRes.SfFltVal > ( UnAdjMin.SfFltVal) ){
		usErrFlg = EXE_AFOFF;
...
////////// ////////// ////////// ////////// //////////
	...
	RegWriteA_LC898122AF( OPGSEL2, ucOPGSEL2 );					// 0x0255
	RegWriteA_LC898122AF( VGA_SW1, ucVGA_SW1 );					// 0x025B
	RamWrite32A_LC898122AF( DAZHLO, ulDAZHLO );					// 0x1529
	RamWriteA_LC898122AF( TREG_H,	usTREG_HL ) ;					// TREG_H(0x0380) - TREG_L(0x0381)
...
unsigned long MesMSABS1AV( void )
{
	...
	RamRead32A_LC898122AF( MSABS1AV, &(ulRamDat) ) ;			// 0x1041
...
void	SetDOFSTDAF( unsigned char ucSetDat )
{
	...
	if( UcCvrCod == CVER122 ){
		RegReadA_LC898122AF( DRVFC3AF, &ucRegDat );
		RegWriteA_LC898122AF( DRVFC4AF, ( ucSetDat & 0x18 ) << 3 );
		RegWriteA_LC898122AF( DRVFC3AF, ( ucRegDat & 0x70 ) | ( ucSetDat & 0x07 ) );
	}
	else{
		RegWriteA_LC898122AF( DRVFC4AF, ucSetDat << 3 );
	}
/* 20141113 Miyake*/
//	WitTim_LC898122AF( AFMID_TIM );							//
}
