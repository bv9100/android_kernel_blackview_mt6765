...
//hct-drv add for Wireless Charge by zhuguanglei 20180320 begin
		...
		static int hct_wireless_charge_gpio_num;
		
		int get_wireless_gpio(void);
		int hct_parse_wireless_dts(struct device_node *node, const char *gpio_name);
	
	#endif
#endif
//hct-drv add for Wireless Charge by zhuguanglei 20180320 end
...
#ifdef __HCT_WIRELESS_CHARGE_SUPPORT__
	#if __HCT_WIRELESS_CHARGE_SUPPORT__
		printk("pull up the wireless gpio = (%d) before enable OTG.\n",hct_wireless_charge_gpio_num);
		gpio_set_value(hct_wireless_charge_gpio_num, 1);
		msleep(1000);
	#endif
#endif
//hct-drv add for Wireless Charge by zhuguanglei 20180320 end
...
//hct-drv add for custom OTG current begin
		#ifdef __HCT_OTG_CURRENT__
			charger_dev_set_boost_current_limit(primary_charger, __HCT_OTG_CURRENT__);
		#else
...
#ifdef __HCT_WIRELESS_CHARGE_SUPPORT__
	#if __HCT_WIRELESS_CHARGE_SUPPORT__
		printk("pull down the wireless gpio = (%d) after disable OTG.\n",hct_wireless_charge_gpio_num);
		gpio_set_value(hct_wireless_charge_gpio_num, 0);
		msleep(1000);
	#endif
#endif
//hct-drv add for Wireless Charge by zhuguanglei 20180320 end
...
int get_wireless_gpio(void)
{
	...
	if (node)
	{
		hct_wireless_charge_gpio_num = hct_parse_wireless_dts(node, WIRELESS_CHARGE_GPIO_NAME);

		if (0 > hct_wireless_charge_gpio_num)
		{
			printk("%s: hct_parse_dts fail. \n", __func__);
			return -1;
		}
	}
	else
	{
		printk("%s: cannot get the node: 'mediatek,mt6761-usb20'.\n", __func__);
		return -ENODEV;
	}

	printk("%s: end. and hct_wireless_charge_gpio_num = (%d)\n", __func__,hct_wireless_charge_gpio_num);
	return 0;
}
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on){
		_set_vbus(1);
		#if defined(CONFIG_USB_MTK_OTG)
			mobile_source_flag = IS_MOBILE_SOURCE_ON_ENABLE;
			printk("otg: is enable!!!\n");
			mobile_source_handler(1);
		#endif
	}else{
		_set_vbus(0);
		#if defined(CONFIG_USB_MTK_OTG)
			mobile_source_flag = IS_MOBILE_SOURCE_OFF;
			printk("otg: is disable!!!\n");
			mobile_source_handler(0);
		#endif
	}
...
//hct-drv add for Wireless Charge by zhuguanglei 20180320 begin
			...
			if(0 > get_wireless_gpio())
				printk("get_wireless_gpio failed.\n");
		#endif
	#endif
//hct-drv add for Wireless Charge by zhuguanglei 20180320 end
