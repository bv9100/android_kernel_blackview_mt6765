...
#include "gc0310_mipi_yuv_sub.h"
#include <linux/hct_include/hct_project_all_config.h>

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC0310_mipi_yuvr"
#define LOG_1 LOG_INF("GC0310, MAIN FAKE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC0310_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#if 1
static SENSOR_FUNCTION_STRUCT sensor_func222 = {
    open,
    get_info,
    get_resolution,
    feature_control,
...
UINT32 NOUSE_YUV_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func222;
    return ERROR_NONE;
}    /*       */
#endif
...
static kal_uint16 GC0310_write_cmos_sensor(kal_uint8 addr, kal_uint8 para)
{
...
static void GC0310_Sensor_Init(void)
{
...
static kal_uint16 GC0310_read_cmos_sensor(kal_uint8 addr)
{
	kal_uint16 get_byte=0;
...
static UINT32 GC0310Open(void)
{
    ...
    for(i=0;i<3;i++)
		...
		{
			printk("gc0310_mipi_yuv_sub.c. GC0310mipi Read Sensor ID ok[open] = 0x%x\n", sensor_id);
			break;
		}
		}
	...
	if(sensor_id != GC0310_SENSOR_ID)
	{
		printk("gc0310_mipi_yuv_sub.c. GC0310mipi Read Sensor ID Fail[open] = 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	GC0310_Sensor_Init();
...
static kal_uint32 GC0310_Read_Shutter(void)
{
...
//printk("GC0310_Read_Shutter_1 0xef=%d\n",temp_reg3);
	shutter = (temp_reg1 & 0xFF) | (temp_reg2 << 8)| (temp_reg3 << 16);
printk("gc0310_mipi_yuv_sub.c. GC0310_Read_Shutter_1 shutter=%d\n",shutter);
	return shutter;
} /* GC0310_read_shutter */

static UINT32 GC0310_GetSensorID(void)
{
    int  retry = 3;
	UINT32 sensorID;
    ...
    do {
        ...
        if (sensorID == GC0310_SENSOR_ID)
            break;
        printk("gc0310_mipi_yuv_sub.c. Read Sensor ID Fail = 0x%04x\n", sensorID);
        retry--;
    } while (retry > 0);
...
SENSOR_FUNCTION_STRUCT_1	SensorFuncGC0310YUV_1_SUB=
{
	GC0310Open,
	GC0310_Read_Shutter,
	GC0310_GetSensorID,
...
UINT32 GC0310_YUV_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
