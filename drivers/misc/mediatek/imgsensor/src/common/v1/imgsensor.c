...
#ifdef SP0A38_MIPI_YUV
#define MainFakeFunc SensorFuncSP0A38YUV_1
#else
#ifdef GC2145_MIPI_YUV
#define MainFakeFunc SensorFuncGC2145YUV_1
#else
...
#ifdef SP0A38_MIPI_YUV
#define SubFakeFunc SensorFuncSP0A38YUV_1
#else
	...
	if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
		{
			if (ret == ERROR_NONE)
			{
		        ...
		        #else
				#ifdef GC2145_MIPI_YUV
				strcpy(buf_yuv_name, SENSOR_DRVNAME_GC2145_MIPI_YUV);
				#else
...
#endif
     			imgsensor_mutex_lock(psensor_inst);
				gc0310_imgsensor_hw_power_sequence(&pgimgsensor->hw, IMGSENSOR_SENSOR_IDX_MAIN_FAKE, IMGSENSOR_HW_POWER_STATUS_ON, sensor_power_sequence, buf_yuv_name);

				if(MainFakeFunc.SensorOpen() == ERROR_NONE)
					pr_err("Main Fake %s SensorOpen Successfull", buf_yuv_name);
				else
		        {
		          if (FACTORY_BOOT == get_boot_mode() || ATE_FACTORY_BOOT == get_boot_mode())
		            ret = ret | ERROR_SENSOR_CONNECT_FAIL;

          		  gc0310_imgsensor_hw_power_sequence(&pgimgsensor->hw, IMGSENSOR_SENSOR_IDX_MAIN_FAKE, IMGSENSOR_HW_POWER_STATUS_OFF, sensor_power_sequence, buf_yuv_name);

				  pr_err("Main Fake %s SensorOpen failed!!!", buf_yuv_name);
       			}
...
#if __HCT_SUB_FAKE_I2C_USE_MAIN__	
{
				...
				if (psensor_subfake != NULL) {
					...
					PK_DBG("[%s] psensor_subfake is NULL.\n", __func__);
				}
}
#endif
        		imgsensor_mutex_lock(psensor_inst);
				gc0310_imgsensor_hw_power_sequence(&pgimgsensor->hw, IMGSENSOR_SENSOR_IDX_SUB_FAKE, IMGSENSOR_HW_POWER_STATUS_ON, sensor_power_sequence, buf_yuv_name);

				if(SubFakeFunc.SensorOpen() == ERROR_NONE)
				{
					pr_err("Sub Fake %s SensorOpen Successfull", buf_yuv_name);
#if __HCT_SUB_FAKE_I2C_USE_MAIN__	
					sub_fake_i2c_flag = 1;
#endif
				}
				else
		        {
		          if (FACTORY_BOOT == get_boot_mode() || ATE_FACTORY_BOOT == get_boot_mode())
          		  ...
          		  gc0310_imgsensor_hw_power_sequence(&pgimgsensor->hw, IMGSENSOR_SENSOR_IDX_SUB_FAKE, IMGSENSOR_HW_POWER_STATUS_OFF, sensor_power_sequence, buf_yuv_name);

				  pr_err("Sub Fake %s SensorOpen failed!!!", buf_yuv_name);
        		}
	...
	MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	struct SENSOR_FUNCTION_STRUCT *psensor_func =  psensor->pfunc;
#if __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
	if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN_FAKE)
	return ERROR_NONE;
#endif

#if __HCT_DUAL_SUB_CAMERA__
	if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_SUB_FAKE)
	return ERROR_NONE;
#endif
...
//hct-drv add begin
	#ifdef CONFIG_HCT_DEVICE_INFO_SUPPORT
	hct_set_camera_device_used(psensor_inst->psensor_name, psensor->inst.sensor_idx+1);
	#endif
//hct-drv add end
...
static int __init imgsensor_init(void)
{
	...
	pr_err("device_create_file vibr_on fail!\n");
#endif
