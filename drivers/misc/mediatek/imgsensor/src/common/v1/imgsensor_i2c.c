...
static struct IMGSENSOR_I2C_CFG gi2c_cfg;
...
#ifdef __HCT_FAKE_CHANGE_I2C__
	gi2c_cfg.pinst       = &gi2c.inst[IMGSENSOR_I2C_DEV_2];
	gi2c_cfg.pi2c_driver = &gi2c_driver[IMGSENSOR_I2C_DEV_2];
	mutex_init(&gi2c_cfg.i2c_mutex);
#endif
	return 0;
}
...
#ifdef __HCT_FAKE_CHANGE_I2C__
	if (hct_i2cflag)
		pgi2c_cfg_legacy = &gi2c_cfg;
	else
		pgi2c_cfg_legacy = pi2c_cfg;
	pr_err("hct-drv-%s: %s %s\n",__func__, pi2c_cfg->pi2c_driver->driver.name, pgi2c_cfg_legacy->pi2c_driver->driver.name);
#else
	pgi2c_cfg_legacy = pi2c_cfg;
#endif
}
