...
static ssize_t  CAMERA_HW_DumpReg_To_Proc5(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
    return 0;
}
#endif
#if  __HCT_DUAL_SUB_CAMERA__
static ssize_t  CAMERA_HW_DumpReg_To_Proc6(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
    return 0;
}
#endif
...
#if __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
static ssize_t  CAMERA_HW_Reg_Debug5(struct file *file, const char *buffer, size_t count,
                                     loff_t *data)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
	struct IMGSENSOR_SENSOR *psensor = &pgimgsensor->sensor[IMGSENSOR_SENSOR_IDX_MAIN_FAKE];
    ...
    if (sscanf(regBuf, "%x %x",  &sensorReg.RegAddr, &sensorReg.RegData) == 2) {
        ...
        pr_debug("write addr = 0x%08x, data = 0x%08x\n", sensorReg.RegAddr, sensorReg.RegData);
    } else if (sscanf(regBuf, "%x", &sensorReg.RegAddr) == 1) {
        imgsensor_sensor_feature_control(psensor, SENSOR_FEATURE_GET_REGISTER, (MUINT8 *)&sensorReg, (MUINT32 *)sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
        pr_debug("read addr = 0x%08x, data = 0x%08x\n", sensorReg.RegAddr, sensorReg.RegData);
    }
...
#if __HCT_DUAL_SUB_CAMERA__
static ssize_t  CAMERA_HW_Reg_Debug6(struct file *file, const char *buffer, size_t count,
                                     loff_t *data)
{
    char regBuf[64] = {'\0'};
    u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
	struct IMGSENSOR_SENSOR *psensor = &pgimgsensor->sensor[IMGSENSOR_SENSOR_IDX_SUB_FAKE];
...
#if __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
static  struct file_operations fcamera_proc_fops5 = {
    .read = CAMERA_HW_DumpReg_To_Proc5,
    .write = CAMERA_HW_Reg_Debug5
};
#endif
#if __HCT_DUAL_SUB_CAMERA__
static  struct file_operations fcamera_proc_fops6 = {
    .read = CAMERA_HW_DumpReg_To_Proc6,
...
#if __HCT_DUAL_CAMERA_USEDBY_YUV_MODE__
  proc_create("driver/camsensor5", 0, NULL, &fcamera_proc_fops5);  
#endif
#if __HCT_DUAL_SUB_CAMERA__
  proc_create("driver/camsensor6", 0, NULL, &fcamera_proc_fops6);   
#endif
	proc_create(
