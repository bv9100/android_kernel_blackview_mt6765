...
UINT32 GC5555_YUV_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
...
UINT32 GC0310Open(void)
{
    ...
    for(i=0;i<3;i++)
		...
		{
			printk("gc0310_mipi_yuv.c. GC0310mipi Read Sensor ID ok[open] = 0x%x\n", sensor_id);
			break;
		}
		}
	...
	if(sensor_id != GC0310_SENSOR_ID)
	{
		printk("gc0310_mipi_yuv.c. GC0310mipi Read Sensor ID Fail[open] = 0x%x\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	GC0310_Sensor_Init();
...
printk("gc0310_mipi_yuv.c. GC0310_Read_Shutter_1 shutter=%d\n",shutter);
	return shutter;
} /* GC0310_read_shutter */
...
UINT32 GC0310_GetSensorID(void)
{
    ...
    do {
        ...
        if (sensorID == GC0310_SENSOR_ID)
            break;
        printk("gc0310_mipi_yuv.c. Read Sensor ID Fail = 0x%04x\n", sensorID);
        retry--;
    } while (retry > 0);
