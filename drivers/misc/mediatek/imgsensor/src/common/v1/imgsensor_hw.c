...
char *imgsensor_sensor_idx_name[IMGSENSOR_SENSOR_IDX_MAX_NUM] = {
	...
	IMGSENSOR_SENSOR_IDX_NAME_MAIN_FAKE		
	IMGSENSOR_SENSOR_IDX_NAME_SUB_FAKE		
};
...
enum IMGSENSOR_RETURN gc0310_imgsensor_hw_power_sequence(
	struct IMGSENSOR_HW             *phw,
	enum   IMGSENSOR_SENSOR_IDX      sensor_idx,
	enum   IMGSENSOR_HW_POWER_STATUS pwr_status,
	struct IMGSENSOR_HW_POWER_SEQ   *ppower_sequence,
	...
	while(ppwr_info->pin != IMGSENSOR_HW_PIN_NONE &&
		...
		if(pwr_status == IMGSENSOR_HW_POWER_STATUS_ON &&
		   ppwr_info->pin != IMGSENSOR_HW_PIN_UNDEF) {
			...
			pr_info("sensor_idx = %d, pin=%d, pin_state_on=%d, hw_id =%d(0:PMIC, 1:GPIO, 2:exGPIO)\n",
