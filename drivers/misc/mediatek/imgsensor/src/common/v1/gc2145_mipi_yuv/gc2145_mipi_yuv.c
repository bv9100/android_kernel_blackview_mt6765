...
#define GC2145_TEST_PATTERN_CHECKSUM 0x54ddf19b 
...
*
*************************************************************************/
static kal_uint16 GC2145_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;

	char pu_send_cmd[1] = {(char)(addr & 0xFF) };
	iReadRegI2C(pu_send_cmd, 1, (u8*)&get_byte, 1, GC2145_READ_ID);
...
*
*************************************************************************/
static void GC2145_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
#if 1
		char pu_send_cmd[2] = {(char)(addr & 0xFF), (char)(para & 0xFF)};
		iWriteRegI2C(pu_send_cmd, 2, GC2145_WRITE_ID);
#else
		iWriteReg((u16)addr, (u32)para, 2, GC2145_WRITE_ID);
#endif
...
static kal_uint16 GC2145_exposure_lines=0;
...
*
*************************************************************************/
static kal_uint32 get_imgsensor_id(UINT32 *sensorID)
...
UINT32 GC2145Open(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		SENSORDB("GC2145_Open, sensor_id:%x [%x, %x] \n",sensor_id,GC2145_read_cmos_sensor(0xf0), GC2145_read_cmos_sensor(0xf1));
...
{
	//kal_uint8 iTemp, temp_AWB_reg;
//	kal_uint16 iDummyPixels = 0, iStartX = 0, iStartY = 0;
...
UINT32 GC2145GetInfo(enum MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 GC2145Control(enum MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	switch (ScenarioId)
	{
...
UINT32 GC2145YUVSensorSetting(enum FEATURE_ID iCmd, UINT32 iPara)
{
...
/*************************************************************************
  ...
  *************************************************************************/
  ...
  UINT32 GC2145SetMaxFramerateByScenario(enum MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	//kal_uint32 pclk;
	//kal_int16 dummyLine;
...
/*************************************************************************
  ...
  *
  *************************************************************************/
UINT32 GC2145GetDefaultFramerateByScenario(enum MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
...
static void GC2145FlashTriggerCheck(unsigned int *pFeatureReturnPara32) 
{ 
	unsigned int ag; 
	 
	GC2145_write_cmos_sensor(0xfe,0x00);
	ag = GC2145_read_cmos_sensor(0x25);
...
{
    SENSORDB("[GC2145SetTestPatternMode]test pattern bEnable:=%d\n",bEnable);
    if(bEnable)
    {
	GC2145_write_cmos_sensor(0xfe, 0x00); 
	GC2145_write_cmos_sensor(0xb6, 0x00); 
	...
	GC2145_write_cmos_sensor(0x03, 0x06); 
	GC2145_write_cmos_sensor(0x04, 0x40); 
	GC2145_write_cmos_sensor(0x40, 0x00); 
	GC2145_write_cmos_sensor(0x5e, 0x00);  
	GC2145_write_cmos_sensor(0x5f, 0x00);  
	GC2145_write_cmos_sensor(0x60, 0x00);  
	GC2145_write_cmos_sensor(0x61, 0x00);  
	...
	GC2145_write_cmos_sensor(0x64, 0x00);  
	GC2145_write_cmos_sensor(0x65, 0x00); 
	GC2145_write_cmos_sensor(0x66, 0x00); 
	GC2145_write_cmos_sensor(0x67, 0x00); 
	GC2145_write_cmos_sensor(0x68, 0x00); 
	GC2145_write_cmos_sensor(0x69, 0x00); 
	GC2145_write_cmos_sensor(0xb0, 0x60); 
	GC2145_write_cmos_sensor(0xb1, 0x20); 
	GC2145_write_cmos_sensor(0xb2, 0x40); 
	GC2145_write_cmos_sensor(0xb3, 0x40); 
	GC2145_write_cmos_sensor(0xb4, 0x40); 
	GC2145_write_cmos_sensor(0xb5, 0x40); 
	GC2145_write_cmos_sensor(0xfe, 0x02); 
	GC2145_write_cmos_sensor(0x3c, 0x00); 
	GC2145_write_cmos_sensor(0x4b, 0x00);  
	GC2145_write_cmos_sensor(0x49, 0x00);
	...
	switch (FeatureId)
	{
		...
		break;
   		case SENSOR_FEATURE_GET_TRIGGER_FLASHLIGHT_INFO: 		
			GC2145FlashTriggerCheck(pFeatureData32); 
		break;
		...
		case SENSOR_FEATURE_CHECK_SENSOR_ID:
			 get_imgsensor_id(pFeatureData32);
			 break;
		case SENSOR_FEATURE_SET_YUV_CMD:
		       printk("GC2145 YUV sensor Setting:%d, %d \n", *pFeatureData32,  *(pFeatureData32+1));
			GC2145YUVSensorSetting((enum FEATURE_ID)*feature_data, *(feature_data+1));
		break;
...
kal_uint32 GC2145_Read_Shutter(void)
{
	...
	GC2145_write_cmos_sensor(0xfe , 0x01);		//page1
	temp_reg3 = GC2145_read_cmos_sensor(0xef);
	GC2145_write_cmos_sensor(0xfe , 0x00);

//printk("GC2145_Read_Shutter_1 0xef=%d\n",temp_reg3);
	shutter = (temp_reg1 & 0xFF) | (temp_reg2 << 8)| (temp_reg3 << 16);
printk("gc2145_mipi_yuv.c. GC2145_Read_Shutter_1 shutter=%d\n",shutter);
	return shutter;
} 

UINT32 GC2145_GetSensorID(void)
{
    int  retry = 3;
	UINT32 sensorID;
    // check if sensor ID correct
    do {
        ...
        if (sensorID == GC2145_SENSOR_ID)
            break;
        printk("gc2145_mipi_yuv.c. Read Sensor ID Fail = 0x%04x\n", sensorID);
        retry--;
    } while (retry > 0);
...
SENSOR_FUNCTION_STRUCT_1 SensorFuncGC2145YUV_1=
{
	GC2145Open,
	GC2145_Read_Shutter,
	GC2145_GetSensorID,
...
struct SENSOR_FUNCTION_STRUCT	sensor_func=
{
	GC2145Open,
	GC2145GetInfo,
	GC2145GetResolution,
...
UINT32 GC2145_YUV_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
