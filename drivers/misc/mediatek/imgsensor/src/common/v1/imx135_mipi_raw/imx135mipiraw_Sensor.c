...
static imgsensor_info_struct imgsensor_info = {
    ...
    .pre = {
        ...
        .max_framerate = 300,
		.mipi_pixel_rate = 225000000,
    },
    .cap = {   // 30  fps  capture
        .pclk = 391850000,			//432000000,
        .linelength = 4572,
        .framelength = 3146,
        .startx = 0,
        .starty = 0,
...
{{ 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Preview 2112*1558
 { 4208, 3120, 0000, 0000, 4208, 3120, 4208, 3120, 0000, 0000, 4208, 3120, 0007, 0007, 4208, 3120}, // capture 4206*3128
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // video
 { 4208, 3120,  824,  840, 2560, 1440, 1280,  720, 0000, 0000, 1280,  720, 0000, 0000, 1280,  720}, // hight speed video
 { 4208, 3120,   24,  390, 4160, 2340, 1280,  720, 0000, 0000, 1280,  720, 0000, 0000, 1280,  720}, // slim video

 };// slim video
#define IMX135_OTP_Enable 0
...
LOG_INF("E! currefps:%d\n",currefps);
    ...
    if(currefps == 300) // default 30.0 fps
    {
        ...
        write_cmos_sensor(0x0301,0x05);
        write_cmos_sensor(0x0303,0x01);
        write_cmos_sensor(0x0305,0x0B);		//0x0C
        write_cmos_sensor(0x0309,0x05);
        write_cmos_sensor(0x030B,0x01);
        write_cmos_sensor(0x030C,0x01);		//0x01	//0x02
        write_cmos_sensor(0x030D,0xC1); 	//0xea	//0x1c
        write_cmos_sensor(0x030E,0x01);
        write_cmos_sensor(0x3A06,0x11);
    }
    else
...
if(imgsensor.update_sensor_otp_lsc != 0){}
    ...
    //Sizesetting
    write_cmos_sensor(0x0340,0x0C);
    write_cmos_sensor(0x0341,0x66);		//0x4A
    write_cmos_sensor(0x0342,0x11);
    write_cmos_sensor(0x0343,0xDC);
    write_cmos_sensor(0x0344,0x00);
    write_cmos_sensor(0x0345,0x00);
    ...
    if(currefps == 300)
    {
        write_cmos_sensor(0x0830,0x87);		//0x8f
        write_cmos_sensor(0x0831,0x3F);		//0x47
        write_cmos_sensor(0x0832,0x77);		//0x7f
        write_cmos_sensor(0x0833,0x47);		//0x4F
        write_cmos_sensor(0x0834,0x47);
        write_cmos_sensor(0x0835,0x57);		//0x5f
        write_cmos_sensor(0x0836,0xf7);		//0xff
        write_cmos_sensor(0x0837,0x4f);
        write_cmos_sensor(0x0839,0x1F);
        write_cmos_sensor(0x083A,0x17);
        write_cmos_sensor(0x083B,0x02);
        write_cmos_sensor(0x0202,0x0C);
        write_cmos_sensor(0x0203,0x62);		//0x46
...
static void normal_video_setting(kal_uint16 currefps)    // VideoFullSizeSetting
{
    ...
    write_cmos_sensor(0x0301,0x05);
    write_cmos_sensor(0x0303,0x01);
    write_cmos_sensor(0x0305,0x0c);		//0x0b
    write_cmos_sensor(0x0309,0x05);
    write_cmos_sensor(0x030B,0x01);
    write_cmos_sensor(0x030C,0x01);
    write_cmos_sensor(0x030D,0xb3);		//0x09
    write_cmos_sensor(0x030E,0x01);
    write_cmos_sensor(0x3A06,0x11);
    //Modesetting
    mdelay(10);
...
if(imgsensor.update_sensor_otp_lsc != 0){}
    ...
    //GlobalTimingSetting
    write_cmos_sensor(0x0830,0x6f); // 7f
    write_cmos_sensor(0x0831,0x27); // 37
    write_cmos_sensor(0x0832,0x4f); // 67
    write_cmos_sensor(0x0833,0x2F); // 3f
    write_cmos_sensor(0x0834,0x2f); // 3f
    write_cmos_sensor(0x0835,0x2f); // 47
    write_cmos_sensor(0x0836,0x9f); // df
    write_cmos_sensor(0x0837,0x37); // 47
    write_cmos_sensor(0x0839,0x1F); // 1f
    write_cmos_sensor(0x083A,0x17); // 17
    write_cmos_sensor(0x083B,0x02); // 2
    //IntegrationTimeSetting
    write_cmos_sensor(0x0202,0x06);
    write_cmos_sensor(0x0203,0x64);// 46
...
if(imgsensor.update_sensor_otp_lsc != 0){}
    ...
    write_cmos_sensor(0x3312,0x02);//
    write_cmos_sensor(0x3313,0xD0);//
    write_cmos_sensor(0x331C,0x02);//
    write_cmos_sensor(0x331D,0x18);//
    ...
    write_cmos_sensor(0x0831,0x17);
    write_cmos_sensor(0x0832,0x37);
    write_cmos_sensor(0x0833,0x17);
    write_cmos_sensor(0x0834,0x17);
    write_cmos_sensor(0x0835,0x0F);
    write_cmos_sensor(0x0836,0x4F);
