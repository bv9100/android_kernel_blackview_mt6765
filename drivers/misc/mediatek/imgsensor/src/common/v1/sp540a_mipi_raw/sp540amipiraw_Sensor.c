...
#include "sp540amipiraw_Sensor.h"
/****************************Modify Following Strings for Debug****************************/
#define PFX "SP540a_camera_sensor"
#define LOG_1 LOG_INF("SP540a,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP540A_SENSOR_ID,

    .checksum_value = 0xf7375923,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 75000000,              //record different mode's pclk
        .linelength = 1885,             //record different mode's linelength
        .framelength = 1985,            //record different mode's framelength
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_B,//sensor output first pixel color//B
    .mclk = 24,//mclk value, suggest 24 or 26 for 24Mhz or 26Mhz
    .mipi_lane_num = SENSOR_MIPI_2_LANE,//mipi lane num
    .i2c_addr_table = {0x78,0x79,0xff}, //5409H---0x78 /5409---0x7a  zhanghua
    .i2c_speed = 200,
};
...
{{ 2592, 1944, 0000, 0000, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // Preview 2112*1558
 { 2592, 1944, 0000, 0000, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // capture 4206*3128
 { 2592, 1944, 0000, 0000, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // video 
 { 2592, 1944, 0000, 0000, 2592, 1944,  648,  486, 0000, 0000,  648,  486, 0004, 0003,  640,  480}, //hight speed video 
 { 2592, 1944, 0000, 0000, 2592, 1944, 1296,  972, 0000, 0000, 1296,  972, 0000, 0000, 1296,  972},// slim video
};
   	 ...
   	 {   
   	    	reg_gain = 0x10 * gain/BASEGAIN ;        //change mtk gain base to aptina gain base
   	    	 ...
   	    	 if(reg_gain<=0x10)
   	    	 {
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
 //  	    	    	LOG_INF("SP540a reg_gain =%d, SP540aMIPI Gain = %d", reg_gain, gain);
   	    	 }
   	    	 else if(reg_gain>= 0xa0)
   	    	 {
   	    	    	 write_cmos_sensor(0xfd, 0x01);
   	    	    	 write_cmos_sensor(0x24,0xa0);
   	    	    	 write_cmos_sensor(0x01, 0x01);
  // 	    	    	 LOG_INF("SP540aMIPI reg_gain =%d, SP540aMIPI Gain = %d",  reg_gain, gain);
	        }        	
   	    	 else
   	    	 {
   	    	    	 write_cmos_sensor(0xfd, 0x01);
...
write_cmos_sensor(0x04,0x86);
write_cmos_sensor(0x24,0xff);
write_cmos_sensor(0x06,0x10);    
write_cmos_sensor(0x09,0x02);
write_cmos_sensor(0x0a,0x20);
write_cmos_sensor(0x01,0x01);
write_cmos_sensor(0x11,0x30);
write_cmos_sensor(0x13,0x44);
write_cmos_sensor(0x28,0x42);
write_cmos_sensor(0x16,0x00);
write_cmos_sensor(0x19,0x23);
write_cmos_sensor(0x1c,0x16);
write_cmos_sensor(0x1e,0x18);
write_cmos_sensor(0x21,0x26);
write_cmos_sensor(0x25,0x01);
write_cmos_sensor(0x26,0x0b);
write_cmos_sensor(0x2c,0x20);
write_cmos_sensor(0x2e,0x02);
write_cmos_sensor(0x2f,0x03);
write_cmos_sensor(0x51,0x30);
write_cmos_sensor(0x52,0x2a);
write_cmos_sensor(0x57,0x30);
write_cmos_sensor(0x66,0x8a);
write_cmos_sensor(0x68,0x6e);
write_cmos_sensor(0x71,0xa0);
write_cmos_sensor(0xe5,0x33);
write_cmos_sensor(0xe6,0x73);
write_cmos_sensor(0xd0,0x00);
write_cmos_sensor(0xfb,0x73);     //73
write_cmos_sensor(0xf0,0x00);    //blacklevel  offset
write_cmos_sensor(0xf1,0x00);    //blacklevel  offset
write_cmos_sensor(0xf2,0x00);    //blacklevel  offset
write_cmos_sensor(0xf3,0x00);    //blacklevel  offset
write_cmos_sensor(0xfd,0x01);   
write_cmos_sensor(0xa1,0x04);
write_cmos_sensor(0xcd,0x50);
write_cmos_sensor(0xc1,0x0b); 
write_cmos_sensor(0xc4,0x04); 
write_cmos_sensor(0x9d,0xab);     //"2b"连续   “ab”非连续
write_cmos_sensor(0xb1,0x03);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {
             LOG_INF("superpix i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
UINT32 SP540A_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    SP540aMIPISensorInit    */
