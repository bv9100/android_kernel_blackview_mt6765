...
#define S5K3P3SX_OTP_FUNCTION  0
...
#if S5K3P3SX_OTP_FUNCTION
extern bool read_3P3_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_3P3_eeprom_wb(BYTE* data);

extern  int s5k3p3_otp_read(void);
#endif
extern int read_3P3_eeprom_lenid_byid(void);
#if 0
...
static kal_uint16 read_cmos_sensor_temp(kal_uint16 addr)
{
    kal_uint16 get_byte=0;
...
int s5k3p3sx_is_otp_data(void);
int s5k3p3sx_is_lensid_data(void);

int s5k3p3sx_is_otp_data(void)
{
	LOG_INF("3p3sx_otp_addr_0x0000 = %x\n",read_cmos_sensor_temp(0x0000));
    if(0x01 == read_cmos_sensor_temp(0x0000))
	{
        //LOG_INF("OTP DATA \n");
        return 0;
    }
...
int s5k3p3sx_is_lensid_data(void)
{
	LOG_INF("3p3sx_otp_addr_0x0005 = %x\n",read_cmos_sensor_temp(0x0005));
    if(0x03 == read_cmos_sensor_temp(0x0005))
	{
        //LOG_INF("OTP DATA \n");
        return 0;
    }
...
mdelay(5);         // Wait value must be at least 20000 MCLKs
...
#if S5K3P3SX_OTP_FUNCTION
//WB OTP start
#define RGr_ratio_Typical 0x218
#define BGr_ratio_Typical 0x297
#define GbGr_ratio_Typical 0x403
typedef struct S5K3P3SX_MIPI_otp_struct{
	kal_uint16 RGr_ratio;
	kal_uint16 BGr_ratio;
	kal_uint16 GbGr_ratio;
}S5K3P3SX_OTP_TYPE;

static BYTE g_wb_ratio[6] = {0}; 	// wb otp data
static int g_wb_ret = 0;		// wb otp read status

static int S5K3P3SX_MIPI_read_otp_wb(S5K3P3SX_OTP_TYPE *otp)
{	
   	kal_uint16 RGr_ratio=0, BGr_ratio=0, GbGr_ratio=0;
	int ret = 0;
   	if (g_wb_ret)
	{
	  	RGr_ratio = (g_wb_ratio[0]<<8) | g_wb_ratio[1];
	 	BGr_ratio = (g_wb_ratio[2]<<8) | g_wb_ratio[3];
	  	GbGr_ratio = (g_wb_ratio[4]<<8) | g_wb_ratio[5];
...
static void S5K3P3SX_MIPI_write_otp_wb(S5K3P3SX_OTP_TYPE *otp)
{
	...
	if(Gb_gain>0x100)
	{
  	...
  	LOG_INF("S5K3P3SX_OTP:Gr_gain=0x%x\n",Gr_gain);
	LOG_INF("S5K3P3SX_OTP:R_gain=0x%x\n",R_gain);
	LOG_INF("S5K3P3SX_OTP:B_gain=0x%x\n",B_gain);
	LOG_INF("S5K3P3SX_OTP:Gb_gain=0x%x\n",Gb_gain);
	LOG_INF("S5K3P3SX_OTP:End.\n");
}

static void S5K3P3SX_MIPI_update_wb_register_from_otp(void)
{
   S5K3P3SX_OTP_TYPE current_otp;  
   
   if(S5K3P3SX_MIPI_read_otp_wb(&current_otp))//read otp awb  zp
   		S5K3P3SX_MIPI_write_otp_wb(&current_otp);//apply otp awb
   	return;
}
//WB OTP end
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	lens_id = read_3P3_eeprom_lenid_byid();
	if((s5k3p3sx_is_otp_data()==-1) || (s5k3p3sx_is_lensid_data()==-1)){
		LOG_INF("Read otp flag id fail, 3p3sx_otp_flag=%d, 3p3sx_otp_lensid_flag=%d\n", s5k3p3sx_is_otp_data(),s5k3p3sx_is_lensid_data());
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001))+1;
			LOG_INF("sensor_id = 0x%x, lenid = 0x%x\n", *sensor_id, lens_id);
...
#if S5K3P3SX_OTP_FUNCTION
				s5k3p3_otp_read();//zy
				g_wb_ret = read_3P3_eeprom_wb(g_wb_ratio);
#endif
								
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, i2c: 0x%x, sensor_id = 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	sensor_init();
#if S5K3P3SX_OTP_FUNCTION
	S5K3P3SX_MIPI_update_wb_register_from_otp();
#endif
	spin_lock(&imgsensor_drv_lock);
