...
#include "sp8409mipiraw_Sensor.h"

#define PFX "sp8409_sensor"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
#define SP8409_MaxGain 16

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP8409_SENSOR_ID,
	...
	.i2c_addr_table = {0x40,0x44,0x45,0xff},
...
{
    return (((read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16)) + 0x7BC3);
}
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    if (gain < BASEGAIN || gain > SP8409_MaxGain * BASEGAIN) {
        LOG_INF("Error gain setting");

        if (gain < BASEGAIN)
            gain = BASEGAIN;
        else if (gain > SP8409_MaxGain * BASEGAIN)
            gain = SP8409_MaxGain * BASEGAIN;
    }
...
UINT32 SP8409_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	SP8409_MIPI_RAW_SensorInit	*/
