...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] ={
 ...
 { 2304, 1728,	510,   504, 1280, 720, 1280,  720, 0000, 0000, 1280,  720,      0,	0, 1280, 720}, /*hight speed video*/
...
{
/*Preview 2304*1728 30fps 24M MCLK 4lane 720Mbps/lane*/
	/*preview 30.01fps*/
	LOG_INF("E\n");
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x6010, 0x0001);
	...
	write_cmos_sensor(0x6214, 0x7971);
	write_cmos_sensor(0x6218, 0x0100);
	write_cmos_sensor_8(0x0100, 0x00); /* Streaming off*/
	write_cmos_sensor(0x602A, 0xF408);
	write_cmos_sensor(0x6F12, 0x0048);
	write_cmos_sensor(0x602A, 0xF40C);
	write_cmos_sensor(0x6F12, 0x0000);
	...
	write_cmos_sensor(0x6F12, 0x0001);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x0344, 0x000A);
	write_cmos_sensor(0x0346, 0x000A);
	write_cmos_sensor(0x0348, 0x0909);
	write_cmos_sensor(0x034A, 0x06C9);
...
LOG_INF("E! currefps:%d\n", currefps);
	/*full size 30fps*/
	/*capture setting 2304*1728  720M/lane*/
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x6010, 0x0001);
	mdelay(3);
	write_cmos_sensor(0x6028, 0x4000);
	...
	if (currefps == 300) {
		...
		write_cmos_sensor(0x0800, 0x0000);
		write_cmos_sensor(0x0200, 0x0200);
		write_cmos_sensor(0x0202, 0x0557);
		write_cmos_sensor(0x021C, 0x0200);
		write_cmos_sensor(0x021E, 0x0557);
		write_cmos_sensor(0x0342, 0x141C);
		write_cmos_sensor(0x0340, 0x0DFF);
		write_cmos_sensor(0x3072, 0x03C0);
		write_cmos_sensor(0x6214, 0x7970);
		write_cmos_sensor_8(0x0101, 0x00);
		write_cmos_sensor_8(0x0100, 0x01); /* Streaming on*/
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x6028, 0x2000);
	write_cmos_sensor(0x602A, 0x026C);
	write_cmos_sensor(0x6F12, 0x42C0);
	write_cmos_sensor(0x6F12, 0x0000);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x602A, 0x37D4);
	...
	write_cmos_sensor(0x602A, 0x3804);
	write_cmos_sensor(0x6F12, 0x00D7);
	write_cmos_sensor(0x602A, 0x380A);
	...
	write_cmos_sensor(0x6F12, 0x0001);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x0344, 0x0208);
	write_cmos_sensor(0x0346, 0x0202);
	write_cmos_sensor(0x0348, 0x0707);
	write_cmos_sensor(0x034A, 0x04D1);
	...
	write_cmos_sensor(0x021E, 0x0100);
	write_cmos_sensor(0x0342, 0x141C);
	write_cmos_sensor(0x0340, 0x038B);
	write_cmos_sensor(0x0114, 0x0300);
	write_cmos_sensor(0x3072, 0x03C0);
	write_cmos_sensor(0x6214, 0x7970);
	write_cmos_sensor_8(0x0101, 0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if((s5k3p3sp_is_otp_data()==-1) || (s5k3p3sp_is_lensid_data()==-1)){
		LOG_INF("Read otp flag id fail, 3p3sp_otp_flag=%d, 3p3sp_otp_lensid_flag=%d\n", s5k3p3sp_is_otp_data(),s5k3p3sp_is_lensid_data());
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (*sensor_id ==imgsensor_info.sensor_id) {
				...
				#ifdef HCT_3P3SP_OTP_ON
				s5k3p3sp_get_otp_data();
				#endif
...
//kin0603
UINT32 S5K3P3SP_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
