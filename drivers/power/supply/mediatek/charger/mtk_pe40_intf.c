...
int mtk_pe40_init_state(struct charger_manager *pinfo)
{
	...
	if (!pe40->chg_consumer) {
		pe40->chg_consumer =
			charger_manager_get_by_name(&pinfo->pdev->dev,
					"charger_port1");
		if (!pe40->chg_consumer) {
			chr_err("%s: get charger consumer device failed\n",
					__func__);
			goto retry;
		}
	}
	...
	/* disable charger */
	charger_manager_enable_power_path(pe40->chg_consumer,
			MAIN_CHARGER, false);
	if (is_dual_charger_supported(pinfo) == true) {
		charger_dev_is_chip_enabled(pinfo->chg2_dev,
					&chg2_chip_enabled);
