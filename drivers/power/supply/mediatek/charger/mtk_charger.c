...
#ifdef __HCT_M169_BATTERY_BY_AUXADC__
extern int mtk_battery_gm_id;
#endif
	...
	int idx, bool en)
{
	...
	if (!en && consumer->powerpath_disabled[idx] == false)
		consumer->powerpath_disabled[idx] = true;
	else if (en && consumer->powerpath_disabled[idx] == true)
		consumer->powerpath_disabled[idx] = false;
	else {
		pr_info("[%s] already set: %d %d\n", __func__,
			consumer->powerpath_disabled[idx], en);
		return 0;
	}
	...
	list_for_each(pos, phead) {
		ptr = container_of(pos, struct charger_consumer, list);
		if (ptr->powerpath_disabled[idx] == true) {
			en = false;
			break;
		}
		if (list_is_last(pos, phead))
...
{
	struct charger_manager *info = consumer->cm;
#ifdef 		__HCT_POWER_OFF_CHARGING_CLOSE_THERMAL__
	#if 	__HCT_POWER_OFF_CHARGING_CLOSE_THERMAL__
	...
	if (info != NULL) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
		{
			...
			chr_err("%s: [hct-drv]kernel power 0ff charging, close thermal current limit. \n", __func__);
		}
	#endif
#endif
		_mtk_charger_change_current_setting(info);
...
static void charger_check_status(struct charger_manager *info)
{
	...
	bool charging = mtk_is_charger_on(info);
...
#ifdef __HCT_M169_BATTERY_BY_AUXADC__
	if(mtk_battery_gm_id == 0)
		info->data.battery_cv = __HCT_BATTERY_CV0__;
    else
		info->data.battery_cv = __HCT_BATTERY_CV1__;
#endif
	//printk("hct____zzz%d,%d\n",mtk_battery_gm_id, info->data.battery_cv);
