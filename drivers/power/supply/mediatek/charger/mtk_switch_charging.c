...
#ifdef CONFIG_LOAD_SWITCH_PSC5415A
...
static void _disable_all_charging(struct charger_manager *info)
{
	...
	if (rt_pps_is_dc_running(info->algo_pps))
		rt_pps_stop_dc(info->algo_pps);
#endif
}
...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		pdata->input_current_limit =
				info->data.ac_charger_input_current;
#ifdef CONFIG_LOAD_SWITCH_PSC5415A
	  if (battery_get_soc()>98 && pmic_get_charging_current()<400 && psc5415a_get_revision() == 6 ) 
	  	pdata->charging_current_limit = 2052000;
	  else
#endif
		pdata->charging_current_limit =
				info->data.ac_charger_current;
...
#ifdef CONFIG_RICHTEK_PPS_DC
	rt_pps_plugout_reset(info->algo_pps);
#endif
...
static int mtk_switch_chr_pps_dc(struct charger_manager *info)
{
#ifdef CONFIG_RICHTEK_PPS_DC
	struct switch_charging_alg_data *swchgalg = info->algorithm_data;
...
#ifdef CONFIG_RICHTEK_PPS_DC
	if (swchgalg->state != CHR_PPS_DC)
		if (rt_pps_is_dc_rdy(info->algo_pps))
			swchgalg->state = CHR_PPS_DC;
...
//hct-drv add begin.
		case CHR_PPS_DC:
			ret = mtk_switch_chr_pps_dc(info);
			break;
//hct-drv add end.
...
#ifdef CONFIG_RICHTEK_PPS_DC
	/* get pps device */
	info->algo_pps = pps_dev_get_by_name("algo_pps");
	if (!info->algo_pps) {
		pr_err("%s: get algo pps fail\n", __func__);
		return -ENODEV;
	}
#endif
