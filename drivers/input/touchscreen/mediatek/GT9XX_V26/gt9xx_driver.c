...
void gtp_gpio_input(int gpio_type)
{
	...
	if (gpio_type == GTP_IRQ_GPIO)
		tpd_gpio_as_int_bias_dis(GTP_IRQ_GPIO);
#else
	if (gpio_type == GTP_RST_GPIO)
...
#if __HCT_GTP_VIO28_SUPPORT__
	GTP_ERROR("%s:tp vdd use vio28\n", __func__);
#else
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		#if 0//GTP_GESTURE_WAKEUP  //rm by zhoush
            if (DOZE_ENABLED == doze_status)
            {
                break;
            }
...
static int __init tpd_driver_init(void)
{
	...
	if (META_BOOT == get_boot_mode() || ADVMETA_BOOT == get_boot_mode() || ATE_FACTORY_BOOT == get_boot_mode())
	{
		GTP_INFO("GT9 META_BOOT ADVMETA_BOOT ATE_FACTORY_BOOT return, donot init");
		return 0;
	}
