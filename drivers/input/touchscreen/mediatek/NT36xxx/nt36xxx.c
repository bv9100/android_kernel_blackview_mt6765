...
//hct-drv add for fwvno by qhs end
...
static const struct nvt_ts_trim_id_table trim_id_table[] = {
	...
	{.id = {0xFF, 0xFF, 0xFF, 0x26, 0x65, 0x03}, .mask = {0, 0, 0, 1, 1, 1},
		.mmap = &NT36526_memory_map, .carrier_system = 2}
};
...
int32_t get_nt36xx_fw_vno(void)
{
	uint8_t buf[16] = {0};
	int32_t ret = 0;
	...
	if(ts == NULL){
		NVT_ERR("I2C transfer error. ts is null");
		return 0;
	}
	...
	NVT_LOG("[%s]:IC FW Ver = 0x%02X, FW Ver Bar = 0x%02X\n", __func__, buf[1], buf[2]);
			...
			const struct i2c_device_id *id)
{
	...
	NVT_LOG("end\n");
	//hct-drv add for fwvno begin
	hct_fw_num = get_nt36xx_fw_vno();
	sprintf(nvt_device_driver.descript,"Vno:%d", hct_fw_num);
	NVT_LOG("hct-drv:[%s] hct_fw_num = %d\n",__func__, hct_fw_num);
	//hct-drv add for fwvno end
