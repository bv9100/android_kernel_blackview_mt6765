...
#if(_CustomerFunction_)
int bbk_xxx_fw_update(void);
int32_t update_firmware_ArrayCheck(void);

int bbk_xxx_get_fw_version(int which)
{
	uint8_t buf[16] = {0};
	int32_t ret = 0;
	NVT_LOG("++\n");
	...
	ret=buf[1];
	NVT_LOG("IC FW Ver = 0x%02X\n", buf[1]);
	NVT_LOG("--\n");
	
	return ret;	
}

int bbk_xxx_get_header_fw_version(void)
{
	int32_t ret = 0;
	NVT_LOG("++\n");
	ret=update_firmware_ArrayCheck();

	if(ret != -EINVAL)
		ret=FIRMWARE_DATA[FW_BIN_VER_OFFSET];			
...
int32_t update_firmware_ArrayCheck(void)
{
	...
	if (sizeof(FIRMWARE_DATA)!= FW_BIN_SIZE) {
		NVT_ERR("bin file size not match. (%zu)\n", sizeof(FIRMWARE_DATA));
		return -EINVAL;
	}

	// check if FW version add FW version bar equals 0xFF
	if (FIRMWARE_DATA[FW_BIN_VER_OFFSET]+ FIRMWARE_DATA[FW_BIN_VER_BAR_OFFSET] != 0xFF) {
		NVT_ERR("bin file FW_VER + FW_VER_BAR should be 0xFF!\n");
		NVT_ERR("FW_VER=0x%02X, FW_VER_BAR=0x%02X\n", FIRMWARE_DATA[FW_BIN_VER_OFFSET], FIRMWARE_DATA[FW_BIN_VER_BAR_OFFSET]);
		return -EINVAL;
	}

	//<redirect>
		fw_entry->size = FW_BIN_SIZE;
		fw_entry->data = FIRMWARE_DATA;

		NVT_LOG(" After redirect,FW_VER=0x%02X, FW_VER_BAR=0x%02X\n"
...
void Boot_Update_Firmware(struct work_struct *work)
{
...
#if(_CustomerFunction_)
	int32_t retSum=0,reFileVer=0,reICFwVer=0;
	...
	#endif
	nvt_sw_reset_idle();
	retSum = Check_CheckSum();

	NVT_LOG("reFileVer,reICFwVer,ret=(%d,%d)chsum=%d\n",reFileVer,reICFwVer,retSum);
	
	if (retSum < 0) {	// read firmware checksum failed
...
#if(_CustomerFunction_)
//int bbk_xxx_fw_update(const struct firmware *firmware) 
int bbk_xxx_fw_update(void) 
{
	int32_t ret = 0;
