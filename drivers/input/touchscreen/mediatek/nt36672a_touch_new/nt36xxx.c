 ...
 *
 * $Revision: 14653 $
 * $Date: 2017-07-21 15:07:03 +0800 (Fri, 21 Jul 2017) $
 *
...
int32_t CTP_I2C_READ(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	...
	printk("prize NT36672 CTP_I2C_READ address = %d\n",address);
	while (retries < 5) {
		ret = i2c_transfer(client->adapter, msgs, 2);
		if (ret == 2)	break;
		retries++;
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		ts->x_num = 18;
		ts->y_num = 36;
		ts->abs_x_max = 720;//1080;
		ts->abs_y_max = 1500;//2160;
		ts->max_button_num = 0;
...
*/
		...
		for (i = 0; i < ts->max_touch_num; i++) {
			...
			if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
				...
				printk("hct_drv_[%s]input_x=%d, input_y=%d, ts->abs_x_max=%d, ts->abs_y_max=%d\n",__func__,input_x, input_y, ts->abs_x_max, ts->abs_y_max);
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	nvt_get_fw_info();
	sprintf(current_tp_info.chip, "NT36xx");
    sprintf(current_tp_info.id,"0x%04x",chip_id);
    sprintf(current_tp_info.vendor,"NT36xx");
    sprintf(current_tp_info.more,"%d*%d",2160,1080);
     #endif
      printk("Nt36525 %s %d\n",__func__,__LINE__);	
