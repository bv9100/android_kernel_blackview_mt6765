...
*****************************************************************************/
u8 pb_file_ft8006p[] = {
#include "../include/pramboot/FT8006P_Pramboot_V1.3_20180921.i"
};
...
static int fts_ft8006p_upgrade_mode(
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft8006p_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 app_len = 0;
    ...
    ret = fts_ft8006p_upgrade_mode(client, FLASH_MODE_APP, tmpbuf, app_len);
    if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft8006p_lic_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 lic_len = 0;
    ...
    ret = fts_ft8006p_upgrade_mode(client, FLASH_MODE_LIC, tmpbuf, lic_len);
    if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
static int fts_ft8006p_param_upgrade(struct i2c_client *client, u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 param_length = 0;
    ...
    ret = fts_ft8006p_upgrade_mode(client, FLASH_MODE_PARAM, tmpbuf, param_length);
    if (ret < 0) {
        FTS_INFO("fw upgrade fail,reset to normal boot");
        if (fts_fwupg_reset_in_boot(client) < 0) {
            FTS_ERROR("reset to normal boot fail");
