...
int get_focalltech_fw_vno(void){
	int ret;	
	u8 fw_vno;
	...
	if(NULL == fts_data)
	{
		FTS_ERROR("hct-drv: [%s] fail, fts_data is NULL", __func__);
		return fw_vno;
	}

	ret = fts_i2c_read_reg(client, FTS_REG_FW_VER, &fw_vno);
	if(ret < 0){
		fw_vno = 0;
      		FTS_ERROR("hct-drv: [%s] i2c read failed!", __func__);	
	}
...
static void tpd_suspend(struct device *h)
{
	...
	{
	printk("zcy add tpgesture_status is open !!!\n");
