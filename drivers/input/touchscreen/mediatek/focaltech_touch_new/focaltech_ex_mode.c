...
//hct_add start
#ifdef __HCT_TP_GLOVE_FOR_HALL__
int hct_fts_enter_glove_mode(int mode)
{
	int ret;
    	struct fts_ts_data *ts_data = fts_data;
   	struct i2c_client *client;
	...
	if(NULL == fts_data)
	{
		FTS_ERROR("hct_fts_enter_glove_mode: fail, fts_data is NULL");
		return -1;
	}
	#endif
	client = ts_data->client;
	if (mode == 1) {
		FTS_INFO("[hct_Mode]enter glove mode");
		ret = fts_enter_glove_mode(client, true);
		if (ret >= 0) {
			g_fts_mode_flag.fts_glove_mode_flag = true;
		}
		...
		FTS_INFO("[hct_Mode]exit glove mode");
		ret = fts_enter_glove_mode(client, false);
		if (ret >= 0) {
			g_fts_mode_flag.fts_glove_mode_flag = false;
		}
	}
	FTS_INFO("[hct_Mode]glove mode status:  %d", g_fts_mode_flag.fts_glove_mode_flag);
	return g_fts_mode_flag.fts_glove_mode_flag;
}
#endif
//hct_add end
