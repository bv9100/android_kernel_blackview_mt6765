...
//hct-drv add for tpgesture by qhs begin
	...
	#define HCT_GESTURE_ENABLE			_IOW(TOUCH_IOC_MAGIC,	3, int)		//开启手势开关
	#define HCT_GESTURE_GENERATE		_IOR(TOUCH_IOC_MAGIC,	4, char*)	//上报具体手势
	...
	{
	    pr_err("%s:[hct-drv] Start.\n", __func__);
		flag_irq = 1;
		wake_up(&g_Waitq);
		pr_err("%s:[hct-drv] End.\n", __func__);
	}
#endif
#endif
...
struct pinctrl_state *eint_as_int, *eint_output0,
		*eint_output1, *rst_output0, *rst_output1, *eint_as_int_bias_dis; 
#ifdef __HCT_LDO_TP_GESTURE_EN_PIN__
struct pinctrl_state *ldotpgesture_en_high, *ldotpgesture_en_low;
#endif
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	if (IS_ERR(eint_as_int_bias_dis)) {
		...
		TPD_DMESG("Cannot find pinctrl state_eint_as_int_bias_dis!\n");
		return ret;
	}
#ifdef __HCT_LDO_TP_GESTURE_EN_PIN__
	ldotpgesture_en_high = pinctrl_lookup_state(pinctrl1, "ldotpgesture_en_high");
	if (IS_ERR(ldotpgesture_en_high)) {
		ret = PTR_ERR(ldotpgesture_en_high);
		TPD_DMESG("Cannot find pinctrl ldotpgesture_en_high!\n");
		return ret;
	}

	ldotpgesture_en_low = pinctrl_lookup_state(pinctrl1, "ldotpgesture_en_low");
	if (IS_ERR(ldotpgesture_en_low)) {
		ret = PTR_ERR(ldotpgesture_en_low);
		TPD_DMESG("Cannot find pinctrl ldotpgesture_en_low!\n");
		return ret;
	}
#endif
...
#if defined(__HCT_TP_GESTURE_SUPPORT__)
#if __HCT_TP_GESTURE_SUPPORT__
	case HCT_GESTURE_ENABLE:
	    pr_err("%s:[hct-drv] Gesture Enable Start. \n", __func__);
	    printk("%s:[hct-drv] Gesture Enable Start. \n", __func__);
		err = copy_from_user(&ioarg, (unsigned int*)arg, sizeof(unsigned int));
		pr_err("[hct-drv] ioarg=%d",ioarg);
		//spin_lock_irq(&gesture_lock);
		tpgesture_status = ioarg;
		pr_err("[hct-drv] tpgesture_status=%d",tpgesture_status);
		//spin_unlock_irq(&gesture_lock);
		pr_err("%s:[hct-drv] Gesture Enable End. \n", __func__);
		break;

    case HCT_GESTURE_GENERATE:
        pr_err("%s:[hct-drv] Gesture Value Start. \n", __func__);
		flag_irq = 0;
		wait_event_freezable(g_Waitq, 0 != flag_irq);
		data = (void __user *) arg;
		//spin_lock_irq(&gesture_lock);
		pr_err("[hct-drv] Before assign Gesture Value=%s. \n", tpgesture_value);
		strcpy(temp,tpgesture_value);
		//spin_unlock_irq(&gesture_lock);
		pr_err("[hct-drv] After assign Gesture Value=%s. \n", temp);
		err = copy_to_user(data, &temp, sizeof(temp));
		pr_err("%s:[hct-drv] Gesture Value End. \n", __func__);
		break;
#endif
#endif
//hct-drv add for tpgesture by qhs end
...
#if 1 //__HCT_TP_GESTURE_SUPPORT__  //modify by JK for TP must suspend before LCM ,or TP will still working on aboot 200ms after LCM suspend ,is a bug
