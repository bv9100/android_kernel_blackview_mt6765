...
#define KEY_GESTURE_DOUBLECLICK                           KEY_U
...
*****************************************************************************/
//hct-drv layout for tpgesture by qhs begin
#if 0
...
#endif
//hct-drv layout for tpgesture by qhs end
...
int fts_gesture_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    u8 pointnum;
	 int read_bytes = 0;
    //u8 buf[FTS_GESTURE_DATA_LEN*4] = { 0 };
   // struct input_dev *input_dev = ts_data->input_dev;
    //struct fts_gesture_st *gesture = &fts_gesture_data;

    if (!ts_data->suspended || (DISABLE == fts_gesture_data.mode)) {
        return 1;
    }
...
//hct-drv add for tpgesture by qhs end
    ...
    //for (i = 0; i < FTS_GESTURE_POINTS_MAX; i++) {
        index = 4 * i + 4;
        fts_gesture_data.coordinate_x[i] = (u16)(((buf[0 + index] & 0x0F) << 8)
                                         + buf[1 + index]);
        fts_gesture_data.coordinate_y[i] = (u16)(((buf[2 + index] & 0x0F) << 8)
                                         + buf[3 + index]);
    }
...
//hct-drv add for tpgesture by qhs begin
	#if __HCT_TP_GESTURE_SUPPORT__
	hct_check_gesture(0);    //for clear gesture data
	#endif
//hct-drv add for tpgesture by qhs end
