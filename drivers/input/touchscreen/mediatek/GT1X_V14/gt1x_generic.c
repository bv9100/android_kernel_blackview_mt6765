...
extern int hct_hall_flag;
u8 gt1x_config_hallclose[GTP_CONFIG_MAX_LENGTH] = { 0 };
#endif
...
u8 cfgversion = 0; // hct add for cfg version
...
s32 gt1x_send_cfg(u8 * config, int cfg_len)
{
	...
	while (retry++ < 5) {
		...
		if (!ret) {
			mdelay(200);	/* at least 200ms, wait for storing config into flash. */
            mutex_unlock(&mutex_cfg);
...
#ifdef __HCT_TP_GLOVE_FOR_HALL__
	const u8 cfg_grp0_hallclose[] = GTP_CFG_GROUP0_HALLCLOSE;
#endif
	const u8 *cfgs[] = {
...
#ifdef __HCT_TP_GLOVE_FOR_HALL__
	const u8 *cfgs_hallclose[] = {
		cfg_grp0_hallclose, cfg_grp1, cfg_grp2,
		cfg_grp3, cfg_grp4, cfg_grp5
	};
#endif
	u8 cfg_lens[] = {
...
#ifdef __HCT_TP_GLOVE_FOR_HALL__
	memset(gt1x_config_hallclose, 0, sizeof(gt1x_config));
	memcpy(gt1x_config_hallclose, cfgs_hallclose[sensor_id], cfg_len);
#endif
...
void gt1x_select_addr(void)
{
	...
	tpd_gpio_output(GTP_INT_PORT, gt1x_i2c_client->addr == 0x14);
	mdelay(2);
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	if(0 == buf_cfgversion[0])
	{
		ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, buf_cfgversion, 1);
		if (ret < 0) 
			GTP_INFO("Read CFG version fail ret=%d", ret);
		else
			GTP_INFO("Read CFG version=%d", buf_cfgversion[0]);
	}
	...
	cfgversion = buf_cfgversion[0];
	GTP_INFO("IC VERSION:GT%s_%06X(Patch)_%04X(Mask)_%02X(SensorID), cfg version(0x%x)", product_id, patch_id, mask_id >> 8, sensor_id, buf_cfgversion[0]);
...
int get_gt1x_fw_vno(void){
	int ret;	
	u8 fw_vno = 0;

	if(NULL == gt1x_i2c_client){
		GTP_ERROR("hct-drv: [%s] fail, gt1x_i2c_client is NULL", __func__);
		return fw_vno;
	}

	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, &fw_vno, 1);
	if (ret < 0){
		GTP_INFO("hct-drv: [%s] i2c read failed!", __func__);
		fw_vno = 0;
	}

	GTP_INFO("hct-drv: fw_vno=%d", fw_vno);
...
static s32 gt1x_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
...
#endif
		{
			...
			tpd_gpio_output(GTP_INT_PORT, gt1x_wakeup_level);
			mdelay(5);
...
void gt1x_power_reset(void)
{
	...
	if(gt1x_i2c_client == NULL){
		GTP_ERROR("I2C transfer error. gt1x_i2c_client is null");
		return ;
	}
...
#ifdef __HCT_TP_GLOVE_FOR_HALL__
		if(hct_hall_flag){
			GTP_INFO("zzt_hall-open  -->> update gt1x_config ");
#endif
			...
			GTP_INFO("zzt_hall-close  -->> update gt1x_config_hallclose ");
			if(gt1x_send_cfg(gt1x_config_hallclose, gt1x_cfg_length)) {
				mdelay(500);
				continue;
			}
		}
	...
	if (tpd_dts_data.use_tpd_button) 
	{
		if (CHK_BIT(cur_event, BIT_TOUCH_KEY) || CHK_BIT(pre_event, BIT_TOUCH_KEY)) 
		{
			for (i = 0; i < TPD_KEY_COUNT; i++) 
			{
				if (key_value & (0x01 << i)) 
				{
				gt1x_touch_down(tpd_dts_data.tpd_key_dim_local[i].key_x, tpd_dts_data.tpd_key_dim_local[i].key_y, 0,0);
					GTP_DEBUG("Key Down.");
					break;
				}
			}
...
{  
	...
	GTP_ERROR("Bob.chenXXXX gt1x_ps_report.  sensor_data.values[0]= %d. \n", sensor_data.values[0]);				
...
static s32 gt1x_ps_enable(s32 enable)
{
	u8 state;
	s32 ret = -1;
	if(1 == gt1x_halt)//suspend state
	{
		GTP_ERROR("ps suspended enable=%d, gt1x_halt=%d\n", enable, gt1x_halt);
		return 0;
	}
	GTP_ERROR("Bob.chenXXXX gt1x_ps_enable. enable = %d. \n", enable);
...
struct hwm_sensor_data a;
	...
	case SENSOR_GET_DATA:
		GTP_ERROR("Bob.chenXXXX SENSOR_GET_DATA");
		if ((buff_out == NULL) || (size_out < sizeof(a))) {
			GTP_ERROR("Get sensor data parameter error!");
			err = -EINVAL;
		} else {
			sensor_data = (struct hwm_sensor_data *) buff_out;
			//sensor_data->values[0] = gt1x_get_ps_value();
			sensor_data->values[0] = (gt1x_get_ps_value() > 0) ? 10 : 0;
			sensor_data->value_divide = 1;
			GTP_ERROR("Bob.chenXXXX sensor_data->values[0] = %d. \n", sensor_data->values[0]);			
			sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
		}
...
#ifdef PLATFORM_MTK
//#if defined(__HCT_GT1X_V14_PS_USE_POLLING_MODE__)
//#if __HCT_GT1X_V14_PS_USE_POLLING_MODE__
	gt1x_ps_dev->obj_ps.polling = 1;	//0--interrupt mode;1--polling mode;
//#else
...
static struct kobj_attribute sc_attr = 
    __ATTR(state, 0644, smart_cover_show, smart_cover_store);
...
int gt1x_suspend(void)
{
	...
	GTP_INFO("tpgesture_status:%d,gt1x_halt:%d\n",tpgesture_status,gt1x_halt);
	if (tpgesture_status) {
		gesture_enter_doze();
		gt1x_irq_enable();
//		gt1x_halt = 0;
...
#if GTP_GESTURE_WAKEUP
GTP_DEBUG("tpgesture_status:%d,gt1x_halt:%d\n",tpgesture_status,gt1x_halt);
    /* just return 0 if IC does not suspend */
//    if (!tpgesture_status && !gt1x_halt)
    if (!gt1x_halt)
        return 0;
#else
    if (!gt1x_halt)
