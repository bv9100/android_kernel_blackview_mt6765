...
{	
    GTP_DEBUG("hct_check_gesture gesture_id==0x%x\n ",gesture_id);
...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    GTP_DEBUG("gesture 0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
...
#if GTP_GESTURE_WAKEUP
	hct_check_gesture(ges_type);
	//key_code = KEY_GES_CUSTOM;
#else
    mutex_lock(&gesture_data_mutex);
