...
void gt1x_irq_enable(void)
{
	...
	GTP_INFO("%s: already enable", __func__);
	spin_unlock_irqrestore(&irq_lock, flag);
...
void gt1x_irq_disable(void)
{
	...
	GTP_INFO("%s: already disable", __func__);
    	spin_unlock_irqrestore(&irq_lock, flag);
    }
}
...
//	node = of_find_compatible_node(NULL, NULL, "mediatek, TOUCH_PANEL-eint");
	...
	if(node){
		...
		enable_irq_wake(tpd_touch_irq);//@20170707 modify by mtk for fix bug(deep sleep cannot resume) 
		GTP_ERROR("hctwang enable_irq_wake tp!.");
	}else{
...
#endif
//	sprintf(tpd_device_driver.descript,"Vno: %d", gt1x_version.patch_id);
	sprintf(tpd_device_driver.descript,"Vno: %d", cfgversion);// hct add for cfg version
...
{
	gt1x_suspend();
	#ifdef __HCT_GT1X_TP_POWEROFF__
	tpd_off();
	#endif
}
...
static void tpd_resume(struct device *h)
{
	...
	GTP_DEBUG("hct_check_gesture tpgesture_value==%s\n ",tpgesture_value);
	#endif
}
#if GTP_GESTURE_WAKEUP
...
{
	GTP_INFO("zcy add Goodix touch panel driver init.");
