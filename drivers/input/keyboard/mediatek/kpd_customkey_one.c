...
#define CUSTOMKEY_F1_GPIO_NAME  "customkey_f1_gpio"           
#define CUSTOMKEY_F1_NUM 		KEY_F1

static int hct_customkey_f1_irqnum;

static unsigned int hct_customkey_f1_gpio_num;
...
static void kpd_customkey_f1_handler(unsigned long data);
static DECLARE_TASKLET(kpd_customkey_f1_tasklet,kpd_customkey_f1_handler,0);
...
//	int ret = 0;
		if (node)
	{
		if (of_property_read_u32_index(node, gpio_name, 1, &gpio_num)) {
            pr_err("f1 read_irq_registration fail~~~\n");
        }

//		gpio_num = of_get_named_gpio(node, gpio_name, 0);
		if (gpio_num < 0)
		{
			printk("%s: f1 of_get_named_gpio fail. \n", __func__);
			return -1;
		}
		else //获取GPIO成功
		{
			printk("%s: f1 of_get_named_gpio GPIO is %d.\n", __func__, gpio_num);
...
{
	bool pressed;
	int customkey_f1_state = 0;

	printk("%s: f1 enter. \n", __func__);

	customkey_f1_state = gpio_get_value(hct_customkey_f1_gpio_num);//__gpio_get_value(hct_customkey_f1_gpio_num);
	if(customkey_f1_state < 0)
		return;
	printk("%s: f1_num = (%d) customkey_f1_state = (%d) gpio_get_value end. \n", __func__,hct_customkey_f1_gpio_num,customkey_f1_state);

	pressed = !customkey_f1_state;

    customkey_handler(pressed, CUSTOMKEY_F1_NUM);

	printk("%s: %s HW keycode(%u) using EINT =%d.\n", __func__, pressed ? "pressed" : "released", CUSTOMKEY_F1_NUM, customkey_f1_state);

	if (pressed)
	{
		printk("%s: key %s. \n", __func__, pressed ? "pressed" : "released");
		irq_set_irq_type(hct_customkey_f1_irqnum, IRQF_TRIGGER_RISING);
	}
	else
	{
		printk("%s: key %s. \n", __func__, pressed ? "pressed" : "released");
		irq_set_irq_type(hct_customkey_f1_irqnum, IRQF_TRIGGER_FALLING);
	}

	enable_irq(hct_customkey_f1_irqnum);
}

static irqreturn_t hct_customkey_f1_isr(int irqnum, void *data)
{
	printk("%s: enter.\n", __func__);

	disable_irq_nosync(hct_customkey_f1_irqnum);
	tasklet_schedule(&kpd_customkey_f1_tasklet);
	return IRQ_HANDLED;
}
 ...
 */
int hct_customkey_f1_init(struct platform_device *pdev)
{
	int retval;
	//int err = -EIO;
	...
	if (node) {
		...
		printk("%s: f1  cannot get the node: 'mediatek,hct_customkey'.\n", __func__);
		return -ENODEV;
	}
	...
	hct_customkey_f1_gpio_num = hct_parse_dts(node, CUSTOMKEY_F1_GPIO_NAME, false);


	if(hct_customkey_f1_gpio_num < 0)
	{   //获取GPIO num 失败
	...
	{	//打印出GPIO num的值
		printk("%s = %d. \n",CUSTOMKEY_F1_GPIO_NAME, hct_customkey_f1_gpio_num);
	}

	/*irq register*/
	retval = request_irq(hct_customkey_f1_irqnum, hct_customkey_f1_isr, IRQF_TRIGGER_FALLING, "customkeyf1", NULL);
	if (retval != 0) {
		printk("%s: request_irq fail, ret %d, irqnum %d.\n", __func__, retval, hct_customkey_f1_irqnum);
		return retval;
	}
	enable_irq(hct_customkey_f1_irqnum);

	printk("+++++++++++ hct_customkey_f1_init end +++++++++++");
	return retval;
}
