...
//hct-drv add for extamp begin
#ifndef CONFIG_MTK_SPEAKER
extern int extamp_sound_mode;
#endif
//hct-drv add for extamp end
...
#define SPK_WARM_UP_TIME        (55)	/* unit is ms */
	if (enable) {
//hct-drv modify for extamp begin
	#ifdef __HCT_EXTAMP_HP_MODE__
		AudDrv_GPIO_EXTAMP_Select(false, extamp_sound_mode);
	#else
		AudDrv_GPIO_EXTAMP_Select(false, 3);
	#endif
//hct-drv modify for extamp end
		/*udelay(1000); */
		usleep_range(1 * 1000, 2 * 1000);
		AudDrv_GPIO_EXTAMP_Select(true, extamp_sound_mode);//3 //hct-drv modify for extamp
         msleep(SPK_WARM_UP_TIME); //added by mtk
        //usleep_range(5 * 1000, 10 * 1000);//deleted by mtk
	} else {

		AudDrv_GPIO_EXTAMP_Select(false, extamp_sound_mode);//3 //hct-drv modify for extamp
		udelay(500);
	}
}
